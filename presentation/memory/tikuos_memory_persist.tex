...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{lstlisting}[style=cstyle, title={kernel/memory/tiku\_mem.c (portable)}]
...
\multirow{2}{*}{Kernel (portable)}
  ...
  & \texttt{kernel/memory/tiku\_mem.c} & Store logic: find, init, register, R/W \\
\midrule
HAL
  & \texttt{hal/tiku\_mem\_hal.h} & Routes to arch; declares NVM functions \\
\midrule
\multirow{2}{*}{Arch (MSP430)}
  & \texttt{arch/msp430/tiku\_mem\_arch.h} & NVM read/write declarations \\
  & \texttt{arch/msp430/tiku\_mem\_arch.c} & FRAM byte-copy implementations \\
\midrule
\multirow{2}{*}{Tests}
  & \texttt{tests/test\_tiku\_mem.c} & 11 persist test cases + host stubs \\
...
void boot_init(void) {
    ...
    /* store.count tells you how many entries survived the reboot */
}
\end{lstlisting}
...
\textbf{Usage:}
\begin{lstlisting}[style=cstyle]
/* Declare FRAM buffer (on MSP430: __persistent or linker section) */
static uint8_t __persistent cfg_fram[32];

tiku_mem_err_t err = tiku_persist_register(&store, "cfg",
                                            cfg_fram, sizeof(cfg_fram));
\end{lstlisting}
...
static uint8_t __persistent cfg_fram[32];
static uint8_t __persistent cal_fram[16];
static tiku_persist_store_t store;
...
\begin{lstlisting}[style=cstyle]
static uint8_t __persistent counter_fram[4];
...
#define TEST_PERSIST_SMALL_BUF   1
#define TEST_PERSIST_OVERFLOW    1
#define TEST_PERSIST_NOT_FOUND   1
#define TEST_PERSIST_DELETE      1
#define TEST_PERSIST_FULL        1
#define TEST_PERSIST_REBOOT      1
#define TEST_PERSIST_WEAR        1
#define TEST_PERSIST_DUP_KEY     1
\end{lstlisting}
\end{column}
...
\vspace{0.3em}
Host mode provides portable stubs for arch NVM functions ---
\texttt{nvm\_read/write} become \texttt{memcpy}.

\vspace{0.5em}
\begin{block}{Combined Test Binary}
Arena and persistent store tests share one test binary.
All 119 assertions run together.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{lstlisting}[style=cstyle, title={Reboot survival test}]
tiku_persist_store_t store;
uint8_t fram_buf[16];
uint8_t read_buf[16];
const uint8_t data[] = {0x42, 0x43, 0x44};

/* First boot: register and write */
...
tiku_persist_read(&store,
    "boot", read_buf, 16, &len);
TEST_ASSERT(len == 3);
TEST_ASSERT(memcmp(read_buf,
    data, 3) == 0);
\end{lstlisting}
\end{column}
