...
\begin{frame}[fragile]{Example: MCP9808 Temperature Sensor (I2C)}
\begin{lstlisting}[style=cstyle,
  title={I2C sensor read --- MCP9808 at address 0x18 (A0=A1=A2=GND)}]
#define MCP9808_ADDR            0x18
#define MCP9808_REG_TEMP        0x05  /* Ambient temperature register */
#define MCP9808_REG_MANUF_ID    0x06  /* Expected: 0x0054 (Microchip) */
#define MCP9808_REG_DEVICE_ID   0x07  /* Expected: 0x0400 */

/* Read a 16-bit big-endian register */
static int mcp9808_read_reg16(uint8_t reg, uint16_t *value)
{
    uint8_t buf[2];
    int rc = tiku_i2c_write_read(MCP9808_ADDR, &reg, 1, buf, 2);
    if (rc == TIKU_I2C_OK)
        *value = ((uint16_t)buf[0] << 8) | buf[1];
    return rc;
}

/* Parse ambient temperature register (13-bit signed, 0.0625 C/LSB) */
static int mcp9808_read_temp(int16_t *temp_int, uint8_t *temp_frac)
{
    uint16_t raw;
    int rc = mcp9808_read_reg16(MCP9808_REG_TEMP, &raw);
    if (rc != TIKU_I2C_OK) return rc;
    uint8_t upper = (uint8_t)(raw >> 8) & 0x0F;
...
% ---------------------------------------------------------------------------
\begin{frame}{MCP9808 Wiring: MSP430FR5969 LaunchPad}
\begin{center}
\begin{tikzpicture}[
  ...
  \node[box, fill=orange!15, right=5cm of mcu] (sensor) {MCP9808\\{\scriptsize Temp Sensor}};
...
\begin{block}{Notes}
\begin{itemize}
  \item Pull-ups: 4.7\,k$\Omega$ on SDA/SCL to 3.3V
        (most breakouts include these)
  \item A0=A1=A2=GND $\rightarrow$ address \texttt{0x18}
  \item Alert pin: optional, active-low open-drain
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
% ---------------------------------------------------------------------------
\begin{frame}[fragile]{Example 10: MCP9808 Process (examples/10\_mcp9808\_temp/)}
\begin{lstlisting}[style=cstyle, basicstyle=\ttfamily\tiny,
  title={Periodic MCP9808 temperature read --- TikuOS process + event timer}]
...
TIKU_PROCESS_THREAD(mcp9808_process, ev, data)
{
    ...
    /* Init I2C at 100 kHz, verify sensor IDs */
    i2c_cfg.speed = TIKU_I2C_SPEED_STANDARD;
    tiku_i2c_init(&i2c_cfg);
    if (!mcp9808_verify_id()) {          /* Check manuf=0x0054, dev=0x04xx */
        tiku_i2c_close();
        TIKU_PROCESS_EXIT();
    }
    ...
    while (1) {
        ...
        if (mcp9808_read_temp(&temp_int, &temp_frac) == TIKU_I2C_OK) {
            int frac_decimal = (int)((uint16_t)temp_frac * 625 / 100);
            TIKU_PRINTF("[MCP9808] Temp: %d.%d C\n", (int)temp_int,
                        frac_decimal);
...
\midrule
Example
  & \texttt{examples/10\_mcp9808\_temp/}  & MCP9808 I2C temp sensor demo \\
\bottomrule
\end{tabular}
\end{table}
\end{frame}
