...
\begin{frame}[fragile]{Example: Reading a Temperature Sensor (I2C)}
\begin{lstlisting}[style=cstyle,
  title={I2C sensor read --- TMP102 temperature sensor at address 0x48}]
...
static void read_temperature(void)
{
    ...
    /* Write register address, then read 2 bytes (repeated START) */
    rc = tiku_i2c_write_read(TMP102_ADDR, &reg, 1, buf, 2);

    if (rc == TIKU_I2C_OK) {
        /* TMP102: 12-bit result, MSB first, 0.0625 deg C per LSB */
        int16_t raw = ((int16_t)buf[0] << 4) | (buf[1] >> 4);
        TIKU_PRINTF("Temp: %d.%02d C\n", raw / 16, (raw % 16) * 625 / 100);
    } else {
        TIKU_PRINTF("I2C error: %d\n", rc);
    }
...
% ---------------------------------------------------------------------------
\begin{frame}[fragile]{Example: Process-Based Sensor Polling}
\begin{lstlisting}[style=cstyle, basicstyle=\ttfamily\tiny,
  title={Periodic I2C sensor read using TikuOS process + timer}]
#define SENSOR_ADDR 0x48
#define SENSOR_REG  0x00

static struct tiku_timer poll_timer;

TIKU_PROCESS(sensor_process, "I2C Sensor");

TIKU_PROCESS_THREAD(sensor_process, ev, data)
{
    static tiku_i2c_config_t cfg = { .speed = TIKU_I2C_SPEED_STANDARD };
    static uint8_t reg = SENSOR_REG;
    static uint8_t buf[2];

    TIKU_PROCESS_BEGIN();

    tiku_i2c_init(&cfg);
    tiku_timer_set_event(&poll_timer, TIKU_CLOCK_SECOND * 2);

    while (1) {
        TIKU_PROCESS_WAIT_EVENT_UNTIL(ev == TIKU_EVENT_TIMER);

        if (tiku_i2c_write_read(SENSOR_ADDR, &reg, 1, buf, 2) == TIKU_I2C_OK)
            TIKU_PRINTF("Sensor: 0x%x%x\n", buf[0], buf[1]);

        tiku_timer_reset(&poll_timer);  /* drift-free 2s interval */
    }

    TIKU_PROCESS_END();
}
TIKU_AUTOSTART_PROCESSES(&sensor_process);
\end{lstlisting}
\end{frame}
