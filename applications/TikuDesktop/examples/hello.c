...
static int
hello_start(void **state, const tiku_app_services_t *services)
{
    ...
    st->surface = tiku_surface_new(HELLO_W, HELLO_H,
                                        TIKU_C_PANEL);
    if (st->surface == NULL) {
        free(st);
        return -1;
    ...
    tiku_text(st->surface, font, 20, 40 + font->ascent,
                   "Hello from a descriptor.", TIKU_C_TEXT);
    tiku_text(st->surface, tiku_font_plain(), 20,
...
static int
hello_event(void *state, const tiku_event_t *event)
{
    (void)state;
...
main(void)
{
    return tiku_client_run(&tiku_example_hello);
}
#endif
