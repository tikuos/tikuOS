 ...
 *
 * tiku_cff.c - the other kind of outline (see the header).
 *
...
static int
fits(const tiku_cff_t *cff, size_t off, size_t need)
{
 ...
 */
static int
index_read_inner(const tiku_cff_t *cff, size_t off, cff_index_t *out)
{
    size_t last;
 ...
 */
static int
index_read(const tiku_cff_t *cff, size_t off, cff_index_t *out)
{
...
static size_t
index_get(const tiku_cff_t *cff, const cff_index_t *idx, int i,
 ...
 */
static void
dict_walk(const tiku_cff_t *cff, size_t off, size_t len,
 ...
 */
static void
read_private(tiku_cff_t *cff, size_t off, size_t size,
             cff_index_t *subrs)
{
    cff_private_t pv;
...
moveto(cff_run_t *run, float x, float y)
{
    ...
    run->y = y;
    if (run->open) {
        tiku_path_close(run->path);
    }
    tiku_path_move(run->path, tiku_hint_x(run->hint, x),
                        tiku_hint_y(run->hint, y));
    run->open = 1;
}
...
lineto(cff_run_t *run, float x, float y)
{
    ...
    run->x = x;
    run->y = y;
    tiku_path_line(run->path, tiku_hint_x(run->hint, x),
                        tiku_hint_y(run->hint, y));
}
        ...
        float y)
{
    ...
    run->x = x;
    run->y = y;
    tiku_path_cubic(run->path,
                         tiku_hint_x(run->hint, ax),
                         tiku_hint_y(run->hint, ay),
                         tiku_hint_x(run->hint, bx),
                         tiku_hint_y(run->hint, by),
                         tiku_hint_x(run->hint, x),
                         tiku_hint_y(run->hint, y));
}
...
fd_subrs_for(const tiku_cff_t *cff, unsigned gid)
{
...
static void
read_fdarray(tiku_cff_t *cff, size_t off)
{
    cff_index_t fds;
    int i;
 ...
 */
static void
read_vstore(tiku_cff_t *cff, size_t off)
{
    size_t ivs;
...
tiku_cff_t *
tiku_cff_open(const unsigned char *data, size_t len)
{
    tiku_cff_t *cff;
...
void
tiku_cff_close(tiku_cff_t *cff)
{
    free(cff);
}

float
tiku_cff_upem(const tiku_cff_t *cff)
{
    return (cff != NULL) ? cff->upem : 1000.0f;
}

int
tiku_cff_outline(const tiku_cff_t *cff, unsigned gid,
                      const tiku_hint_t *hint, tiku_path_t *path)
{
    cff_run_t run;
