 ...
 *
 * tiku_ttf.c - reading a font somebody dropped in (see the header).
 *
...
static int
fits(const tiku_ttf_t *t, size_t off, size_t need)
{
...
static size_t
table_of(const tiku_ttf_t *t, size_t dir, const char *tag, size_t *len)
{
    unsigned n, i;
...
static void
read_family(tiku_ttf_t *t, size_t name)
{
...
static void
read_cmap(tiku_ttf_t *t, size_t cmap)
{
    unsigned n, i;
    int best = -1;
...
static unsigned
glyph_index(const tiku_ttf_t *t, unsigned cp)
{
...
static int
advance_of(const tiku_ttf_t *t, unsigned g)
{
...
static size_t
glyph_at(const tiku_ttf_t *t, unsigned g, size_t *off)
{
    size_t a, b;
...
static int read_outline(const tiku_ttf_t *t, unsigned g,
                        ttf_outline_t *out, float dx, float dy, int depth);

/** @brief A simple glyph: flags, then x deltas, then y deltas. */
static int
read_simple(const tiku_ttf_t *t, size_t off, size_t len, int contours,
...
static int
read_composite(const tiku_ttf_t *t, size_t off, size_t len,
               ttf_outline_t *out, float dx, float dy, int depth)
{
    size_t p = off + 10u;
    size_t end = off + len;
...
static int
read_outline(const tiku_ttf_t *t, unsigned g, ttf_outline_t *out,
...
static float
glyph_top(const tiku_ttf_t *t, unsigned cp)
{
...
static void
build_hint(tiku_ttf_t *t, float scale, tiku_hint_t *hint)
{
    const char *off = getenv("TIKU_HINT");
 ...
 */
static void
emit_contour(tiku_path_t *path, const ttf_point_t *p, int n,
             const tiku_hint_t *hint)
{
    ...
    tiku_path_move(path, tiku_hint_x(hint, sx),
                        tiku_hint_y(hint, sy));
    for (; i <= n; i++) {
        ...
        if (!on) {
            if (have_ctrl) {
                ...
                tiku_path_quad(path, tiku_hint_x(hint, cx),
                                    tiku_hint_y(hint, cy),
                                    tiku_hint_x(hint,
                                                     0.5f * (cx + qx)),
                                    tiku_hint_y(hint,
        ...
        if (have_ctrl) {
            ...
            tiku_path_line(path, tiku_hint_x(hint, qx),
                                tiku_hint_y(hint, qy));
        }
    }
...
static void
emit_outline(tiku_path_t *path, const ttf_outline_t *outline,
             const tiku_hint_t *hint)
{
    int i, start = 0;
...
int
tiku_ttf_is_font(const char *path)
{
...
tiku_ttf_open(const char *path)
{
    ...
    t->glyf = table_of(t, dir, "glyf", &t->glyf_len);
    {
        ...
        if (cff_off != 0 && cff_len != 0) {
            t->cff = tiku_cff_open(t->data + cff_off, cff_len);
            t->cff_upem = tiku_cff_upem(t->cff);
        }
    }
...
void
tiku_ttf_close(tiku_ttf_t *ttf)
{
    if (ttf != NULL) {
        tiku_cff_close(ttf->cff);
        free(ttf->data);
        free(ttf);
    }
}
...
const char *
tiku_ttf_family(const tiku_ttf_t *ttf)
{
    return (ttf != NULL) ? ttf->family : NULL;
}

int
tiku_ttf_bold(const tiku_ttf_t *ttf)
{
    return (ttf != NULL) ? ttf->bold : 0;
}

void
tiku_ttf_metrics(const tiku_ttf_t *ttf, int px, int *ascent,
                      int *height)
{
    float scale;
...
int
tiku_ttf_has(const tiku_ttf_t *ttf, unsigned cp)
{
    return ttf != NULL && glyph_index(ttf, cp) != 0u;
}

int
tiku_ttf_render(const tiku_ttf_t *ttf, unsigned cp, int px,
                     tiku_ttf_glyph_t *out)
{
    ...
    if (ttf->cff != NULL) {
        ...
        build_hint((tiku_ttf_t *)ttf, (float)px /
                   ((ttf->cff_upem > 0.0f) ? ttf->cff_upem
                                           : (float)ttf->upem), &hint);
        if (!tiku_cff_outline(ttf->cff, g, &hint, path)) {
            tiku_path_free(path);
            return 1;
        }
    } else {
        build_hint((tiku_ttf_t *)ttf, scale, &hint);
        emit_outline(path, &outline, &hint);
    }
    tiku_path_bounds(path, &x0, &y0, &w, &h);
    if (tiku_path_failed(path) || w <= 0 || h <= 0) {
        tiku_path_free(path);
        return 1;               /* nothing drawable: an advance and no ink */
    }
    {
        /* The same switch the grid-fit answers to turns the darkening
         * off as well, so TIKU_HINT=0 gives the honest linear
         * coverage for measuring against. */
        const char *off = getenv("TIKU_HINT");
        int darken = !(off != NULL && off[0] == '0');

        out->cover = tiku_path_render(path, x0, y0, w, h, darken);
    }
    tiku_path_free(path);
    if (out->cover == NULL) {
        return 0;
...
void
tiku_ttf_free_glyph(tiku_ttf_glyph_t *glyph)
{
    if (glyph != NULL) {
        free(glyph->cover);
        glyph->cover = NULL;
