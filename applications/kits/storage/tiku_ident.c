 ...
 *
 * tiku_ident.c - scored device identity.
 *
...
static uint64_t
ident_hash(const tiku_identity_t *id)
{
...
int
tiku_ident_provisional(const tiku_identity_t *id)
{
    return (id == NULL || id->uid[0] == '\0');
}

int
tiku_ident_key(const tiku_identity_t *id, char *out, size_t max)
{
    if (id == NULL || out == NULL || max == 0u) {
        return 0;
    }
    if (!tiku_ident_provisional(id)) {
...
int
tiku_ident_score(const tiku_identity_t *known,
                     const tiku_identity_t *seen)
{
    ...
    if (same(known->uid,  seen->uid))  { s += TIKU_W_UID;  }
    if (same(known->id,   seen->id))   { s += TIKU_W_ID;   }
    if (same(known->mcu,  seen->mcu))  { s += TIKU_W_MCU;  }
    if (same(known->name, seen->name)) { s += TIKU_W_NAME; }
    return s;
}

int
tiku_ident_matches(const tiku_identity_t *known,
                       const tiku_identity_t *seen)
{
    int s = tiku_ident_score(known, seen);

    return (s >= TIKU_BIND) ? 1 : 0;
}
