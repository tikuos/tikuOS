 ...
 *
 * tiku_trk_model.c - the questions the view is allowed to ask.
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include "tiku_trk_model.h"
#include "tiku_trk_strings.h"
...
int
tiku_trk_model_is_container(const tiku_trk_model_t *m)
{
    if (m == NULL) {
        return 0;
    }
    switch (m->kind) {
    case TIKU_TRK_KIND_DIRECTORY:
    case TIKU_TRK_KIND_VOLUME:
    case TIKU_TRK_KIND_ROOT:
    case TIKU_TRK_KIND_VIRTUAL_DIR:
    case TIKU_TRK_KIND_TRASH:
    case TIKU_TRK_KIND_DESKTOP:
    case TIKU_TRK_KIND_QUERY:
        return 1;
    case TIKU_TRK_KIND_SYMLINK:
...
int
tiku_trk_model_is_mutable_container(const tiku_trk_model_t *m)
{
    ...
    if (m->kind == TIKU_TRK_KIND_VIRTUAL_DIR ||
        m->kind == TIKU_TRK_KIND_ROOT ||
        m->kind == TIKU_TRK_KIND_QUERY) {
        return 0;
    }
    return (m->facts.perm & TIKU_TRK_P_WRITE) ? 1 : 0;
}

const char *
tiku_trk_model_display_name(const tiku_trk_model_t *m)
{
    if (m == NULL) {
        return "";
    }
    switch (m->kind) {
    case TIKU_TRK_KIND_ROOT:    return tiku_trk_str("kind.disks");
    case TIKU_TRK_KIND_TRASH:   return tiku_trk_str("kind.trash");
    case TIKU_TRK_KIND_DESKTOP: return tiku_trk_str("kind.desktop");
    default:                    return m->name;
    }
}

int
tiku_trk_model_is_writable(const tiku_trk_model_t *m)
{
    if (m == NULL) {
        return 0;
    }
...
const char *
tiku_trk_model_type_at(int i, const char **label)
{
    if (i < 0 || i >= NTYPES) {
        return NULL;
    }
    if (label != NULL) {
        *label = tiku_trk_str(kTypes[i].kind);
    }
...
int
tiku_trk_model_type_is(const char *type, const char *want)
{
    size_t n;
...
const char *
tiku_trk_model_kind_string(const tiku_trk_model_t *m)
{
    if (m == NULL) {
        return "";
    }
     ...
     * not. */
    {
        ...
        if (r >= 0 && m->kind == TIKU_TRK_KIND_FILE) {
            return tiku_trk_str(kTypes[r].kind);
        }
    }
    switch (m->kind) {
    case TIKU_TRK_KIND_FILE:        return tiku_trk_str("kind.file");
    case TIKU_TRK_KIND_DIRECTORY:   return tiku_trk_str("kind.folder");
    case TIKU_TRK_KIND_SYMLINK:
        return m->facts.link_broken ? "Broken link" : "Symbolic link";
    case TIKU_TRK_KIND_VOLUME:      return tiku_trk_str("kind.volume");
    case TIKU_TRK_KIND_ROOT:        return tiku_trk_str("kind.disks");
    case TIKU_TRK_KIND_VIRTUAL_DIR: return tiku_trk_str("kind.namespace_folder");
    case TIKU_TRK_KIND_DEVICE_NODE: return tiku_trk_str("kind.device_node");
    case TIKU_TRK_KIND_QUERY:       return tiku_trk_str("kind.query");
    case TIKU_TRK_KIND_PRINTER:     return tiku_trk_str("kind.printer");
    case TIKU_TRK_KIND_TRASH:       return tiku_trk_str("kind.trash");
    case TIKU_TRK_KIND_DESKTOP:     return tiku_trk_str("kind.desktop");
    default:                        return tiku_trk_str("kind.unknown");
    }
}
...
const char *
tiku_trk_model_icon_name(const tiku_trk_model_t *m)
{
    if (m == NULL) {
        return "file";
    }
    ...
    case TIKU_TRK_KIND_VOLUME:
        ...
        if (m->facts.vol_flags & TIKU_TRK_VOL_SHARED) {
            return "share";
        }
        return (m->facts.vol_flags & TIKU_TRK_VOL_BOOT) ? "root" : "volume";
    case TIKU_TRK_KIND_ROOT:
        return "root";
    case TIKU_TRK_KIND_PRINTER:
        return "printer";
    case TIKU_TRK_KIND_DEVICE_NODE:
    ...
    case TIKU_TRK_KIND_DIRECTORY:
        ...
        return "folder";
    case TIKU_TRK_KIND_FILE:
    default:
        break;
    }
    if (m->facts.perm & TIKU_TRK_P_EXEC) {
        return "executable";
    }
    {
...
void
tiku_trk_backend_close(tiku_trk_backend_t *b)
{
