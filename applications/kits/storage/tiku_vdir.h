 ...
 *
 * tiku_vdir.h - a directory whose contents are several real ones.
 *
...
#define TIKU_VDIR_MAX_SRC 8

/** @brief One virtual directory: an ordered list of contributing paths. */
typedef struct {
    char src[TIKU_VDIR_MAX_SRC][TIKU_PATH_MAX];
    int  nsrc;
    char def[TIKU_PATH_MAX];    /* the definition file it came from  */
} tiku_vdir_t;

void tiku_vdir_init(tiku_vdir_t *v);
 ...
 */
int tiku_vdir_load(tiku_vdir_t *v, const char *deffile);

/** @brief Add one contributing path at the end (lowest priority). */
int tiku_vdir_add(tiku_vdir_t *v, const char *path);
 ...
 */
int tiku_vdir_owner(const tiku_vdir_t *v, tiku_backend_t *b,
                        const char *name);
 ...
 */
int tiku_vdir_resolve(const tiku_vdir_t *v, tiku_backend_t *b,
 ...
 */
int tiku_vdir_after_change(const tiku_vdir_t *v,
                               tiku_backend_t *b, const char *name,
 ...
 */
int tiku_vdir_list(const tiku_vdir_t *v, tiku_backend_t *b,
...
#define TIKU_VDIR_CH_NONE 0u
#define TIKU_VDIR_CH_DEF  0x1u  /* the definition file was edited     */
#define TIKU_VDIR_CH_SRC  0x2u  /* a contributor's contents moved     */
#define TIKU_VDIR_CH_GONE 0x4u  /* a contributor disappeared          */

/** @brief The last-seen state of everything a virtual directory watches. */
typedef struct {
    int64_t def_mtime;
    int64_t src_mtime[TIKU_VDIR_MAX_SRC];
    int     src_present[TIKU_VDIR_MAX_SRC];
    int     nsrc;
    int     primed;
} tiku_vdir_watch_t;

/** @brief Take the current state as the baseline; reports no change. */
void tiku_vdir_watch_init(tiku_vdir_watch_t *w,
                              const tiku_vdir_t *v);
 ...
 * @return a mask of TIKU_VDIR_CH_*.
 */
unsigned tiku_vdir_poll(const tiku_vdir_t *v,
                            tiku_vdir_watch_t *w, int *gone);
 ...
 */
int tiku_vdir_supplied_by(const tiku_vdir_t *v,
                              tiku_backend_t *b, const char *name,
                              int which);
 ...
 */
void tiku_vdir_drop(tiku_vdir_t *v, int which);

/** @brief Remove a generated definition and its now-empty scratch directory. */
int tiku_vdir_cleanup(tiku_vdir_t *v, const char *scratch_dir);
 ...
 */
uint64_t tiku_vdir_child_id(const tiku_vdir_t *v, const char *name);
 ...
 */
void tiku_vdir_child(const tiku_vdir_t *v, tiku_backend_t *b,
