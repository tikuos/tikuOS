 ...
 */
#ifndef TIKU_TRK_FS_H_
#define TIKU_TRK_FS_H_

#include <stddef.h>
#include <stdint.h>
#include "tiku_trk_model.h"

struct tiku_trk_volumes;
...
typedef enum {
    ...
    TIKU_TRK_OP_COPY,
    TIKU_TRK_OP_MOVE,
    TIKU_TRK_OP_DUPLICATE,
    TIKU_TRK_OP_TRASH,
    TIKU_TRK_OP_DELETE,
    TIKU_TRK_OP_RENAME,
    TIKU_TRK_OP_LINK,
    TIKU_TRK_OP_NEW_FOLDER,
} tiku_trk_op_t;

/** @brief How a name collision was resolved. */
typedef enum {
    TIKU_TRK_CONFLICT_ASK = 0,   /* stop and report; the UI decides       */
    TIKU_TRK_CONFLICT_REPLACE,
    TIKU_TRK_CONFLICT_SKIP,
    TIKU_TRK_CONFLICT_RENAME,    /* invent a fresh name, as Duplicate does */
    ...
    TIKU_TRK_CONFLICT_REPLACE_ALL,
    TIKU_TRK_CONFLICT_SKIP_ALL,
} tiku_trk_conflict_t;
 ...
 */
typedef int (*tiku_trk_cancel_fn)(void *ctx);

typedef struct tiku_trk_fs tiku_trk_fs_t;
 ...
 */
typedef void (*tiku_trk_fs_moved_fn)(void *ctx, const char *from,
                                     const char *to);

/** @brief Listen for items landing somewhere else.  One listener. */
void tiku_trk_fs_set_moved(tiku_trk_fs_t *fs, tiku_trk_fs_moved_fn fn,
                           void *ctx);

/** @brief Create the operation engine for one backend. */
tiku_trk_fs_t *tiku_trk_fs_new(tiku_trk_backend_t *backend);

void tiku_trk_fs_free(tiku_trk_fs_t *fs);
 ...
 * @return TIKU_TRK_OP_MOVE or TIKU_TRK_OP_COPY.
 */
tiku_trk_op_t tiku_trk_fs_drop_action(tiku_trk_fs_t *fs, const char *src,
 ...
 */
int tiku_trk_fs_duplicate_name(tiku_trk_fs_t *fs, const char *dir,
 ...
 */
int tiku_trk_fs_link_name(tiku_trk_fs_t *fs, const char *dir,
                          const char *name, char *out, size_t max);

/** @brief Start copying @p paths into @p dst_dir.  @return 0 if accepted. */
int tiku_trk_fs_copy(tiku_trk_fs_t *fs, const char *const *paths, int n,
                     const char *dst_dir, tiku_trk_conflict_t policy);

/** @brief Start moving.  Items already in @p dst_dir are skipped (FS-005). */
int tiku_trk_fs_move(tiku_trk_fs_t *fs, const char *const *paths, int n,
                     const char *dst_dir, tiku_trk_conflict_t policy);

/** @brief Duplicate in place; each item lands in its OWN parent (FS-008). */
int tiku_trk_fs_duplicate(tiku_trk_fs_t *fs, const char *const *paths, int n);
 ...
 */
int tiku_trk_fs_trash(tiku_trk_fs_t *fs, const char *const *paths, int n);
 ...
 */
typedef enum {
    TIKU_TRK_REFUSE_NONE = 0,
    TIKU_TRK_REFUSE_INTO_SELF,   /* a folder into itself or its own child */
    TIKU_TRK_REFUSE_BY_CHILD,    /* replacing a folder with its own child */
    TIKU_TRK_REFUSE_KIND,        /* a file over a folder, or the reverse   */
    TIKU_TRK_REFUSE_TRASH,       /* the Trash cannot be moved or copied    */
    TIKU_TRK_REFUSE_VOLUME       /* a whole volume is not an item to move  */
} tiku_trk_refusal_t;
 ...
 * @return TIKU_TRK_REFUSE_NONE when the operation may proceed.
 */
tiku_trk_refusal_t tiku_trk_fs_check_move(tiku_trk_fs_t *fs, const char *src,
 ...
 */
int tiku_trk_fs_delete(tiku_trk_fs_t *fs, const char *const *paths, int n);
...
#define TIKU_TRK_TRASH_NAME ".Trash"
 ...
 */
int tiku_trk_fs_trash_dirs(tiku_trk_fs_t *fs, char out[][TIKU_TRK_PATH_MAX],
                           int max);

/** @brief Create and mark every writable mounted volume's Trash at startup. */
int tiku_trk_fs_prepare_trash(tiku_trk_fs_t *fs);
 ...
 */
int tiku_trk_fs_trash_dirs_of(tiku_trk_fs_t *fs,
                              const struct tiku_trk_volumes *vs,
                              char out[][TIKU_TRK_PATH_MAX], int max);

int tiku_trk_fs_trash_dir(tiku_trk_fs_t *fs, const char *path, char *out,
                          size_t max);
 ...
 */
int tiku_trk_fs_has_trash(const tiku_trk_fs_t *fs);
 ...
 */
int tiku_trk_fs_restore(tiku_trk_fs_t *fs, const char *const *paths, int n,
                        char *err, size_t errmax);

/** @brief Where a trashed item came from, or 0 when nothing was recorded. */
int tiku_trk_fs_original_path(tiku_trk_fs_t *fs, const char *path, char *out,
                              size_t max);

/** @brief Empty the Trash.  @return Number removed. */
int tiku_trk_fs_empty_trash(tiku_trk_fs_t *fs);
 ...
 */
typedef enum {
    TIKU_TRK_DELETE_ASK = 0,     /* the three-way question                */
    TIKU_TRK_DELETE_STRAIGHT     /* no question: delete outright          */
} tiku_trk_delete_q_t;

tiku_trk_delete_q_t tiku_trk_fs_delete_question(const tiku_trk_fs_t *fs,
                                                const char *path,
                                                int shift_held);

/** @brief Rename one entry.  @return 0, or -1 with @p err filled. */
int tiku_trk_fs_rename(tiku_trk_fs_t *fs, const char *path,
 ...
 * what the UI shows.
 */
int tiku_trk_fs_name_ok(const char *name, char *err, size_t errmax);
 ...
 */
const char *tiku_trk_fs_name_deny(void);

/** @brief Advance the current operation a little.  @return items processed. */
int tiku_trk_fs_step(tiku_trk_fs_t *fs, int budget);

/** @brief Snapshot of what is happening, for the progress window. */
const tiku_trk_progress_t *tiku_trk_fs_progress(const tiku_trk_fs_t *fs);

/** @brief Answer a conflict the engine stopped on and continue. */
void tiku_trk_fs_resolve(tiku_trk_fs_t *fs, tiku_trk_conflict_t how);

/** @brief Abandon the running operation; what is done stays done. */
void tiku_trk_fs_cancel(tiku_trk_fs_t *fs);
 ...
 * it stands.
 */
void tiku_trk_fs_set_cancel_check(tiku_trk_fs_t *fs, tiku_trk_cancel_fn fn,
                                  void *ctx);
 ...
 */
void tiku_trk_fs_resolve_error(tiku_trk_fs_t *fs, int keep_going);

int tiku_trk_fs_busy(const tiku_trk_fs_t *fs);
...
uint64_t tiku_trk_fs_free_bytes(const char *path);
 ...
 */
uint64_t tiku_trk_fs_items_size(const char *const *paths, int n);
 ...
 */
int tiku_trk_fs_fits(uint64_t total, uint64_t free_bytes);

/** @brief How much ceremony a move of @p path needs (FS-027). */
typedef enum {
    TIKU_TRK_CONFIRM_NONE = 0,
    TIKU_TRK_CONFIRM_ASK,        /* config or settings: a plain question */
    TIKU_TRK_CONFIRM_OVERRIDE    /* system or home: Shift must be held   */
} tiku_trk_confirm_t;
 ...
 */
tiku_trk_confirm_t tiku_trk_fs_confirm_move(const char *path);

/** @brief Whether @p path is the Trash or something inside it (FS-014). */
int tiku_trk_fs_in_trash(tiku_trk_fs_t *fs, const char *path);
 ...
 */
int tiku_trk_fs_link(tiku_trk_fs_t *fs, const char *const *paths, int n,
 ...
 */
void tiku_trk_fs_new_folder_name(tiku_trk_fs_t *fs, const char *dir,
                                 char *out, size_t max);
 ...
 * @return 0 on success.
 */
int tiku_trk_fs_new_folder(tiku_trk_fs_t *fs, const char *dir, char *out,
 ...
 */
int tiku_trk_fs_new_from_template(tiku_trk_fs_t *fs, const char *template_path,
 ...
 */
int tiku_trk_fs_can_undo(const tiku_trk_fs_t *fs, char *what, size_t max);

/** @brief Reverse the last operation.  @return items reversed, or -1. */
int tiku_trk_fs_undo(tiku_trk_fs_t *fs, char *err, size_t errmax);
 ...
 */
int tiku_trk_fs_can_redo(const tiku_trk_fs_t *fs, char *what, size_t max);
 ...
 */
int tiku_trk_fs_last_created(const tiku_trk_fs_t *fs,
                             char out[][TIKU_TRK_PATH_MAX], int max);

/** @brief Replay the operation the last undo reversed. */
int tiku_trk_fs_redo(tiku_trk_fs_t *fs, char *err, size_t errmax);

/** @brief The menu word for @p op ("Copy", "Move to Trash", ...). */
const char *tiku_trk_fs_op_name(tiku_trk_op_t op);

#endif /* TIKU_TRK_FS_H_ */
