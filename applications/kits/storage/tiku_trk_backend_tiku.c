 ...
 *
 * tiku_trk_backend_tiku.c - a live device as a browsable volume.
 *
...
#include "tiku_desk_ns.h"
#include "tiku_desk_session.h"
...
typedef struct {
    ...
    tiku_desk_session_t *sess;
    tiku_desk_ns_t      *ns;
    tiku_trk_store_t    *store;
    tiku_trk_identity_t  ident;
    char                 devid[TIKU_TRK_DEVID_MAX];
} tiku_impl_t;

/** @brief Read one identity node, empty string when absent. */
static void
read_ident_field(tiku_desk_ns_t *ns, const char *path, char *out, size_t max)
{
    const tiku_desk_node_t *n;

    out[0] = '\0';
    if (tiku_desk_ns_read(ns, path) != 0) {
        return;
    }
    n = tiku_desk_ns_find(ns, path);
...
static tiku_trk_kind_t
kind_of(const tiku_desk_node_t *n)
{
    if (n->path[0] == '/' && n->path[1] == '\0') {
        return TIKU_TRK_KIND_VOLUME;
    }
    if (in_store(n->path)) {
        ...
        return n->is_dir ? TIKU_TRK_KIND_DIRECTORY : TIKU_TRK_KIND_FILE;
    }
    return n->is_dir ? TIKU_TRK_KIND_VIRTUAL_DIR : TIKU_TRK_KIND_DEVICE_NODE;
}

/** @brief Copy a namespace node into a Model. */
static void
from_node(tiku_trk_backend_t *b, const tiku_desk_node_t *n,
          tiku_trk_model_t *m)
{
    ...
    m->backend = b;
    m->facts.perm = 0u;
    if (n->perm & TIKU_DESK_P_READ)  { m->facts.perm |= TIKU_TRK_P_READ; }
    if (n->perm & TIKU_DESK_P_WRITE) { m->facts.perm |= TIKU_TRK_P_WRITE; }
     ...
     * on the path.) */
    if (m->kind == TIKU_TRK_KIND_DIRECTORY) {
        m->facts.perm |= TIKU_TRK_P_READ | TIKU_TRK_P_WRITE;
    } else if (m->kind == TIKU_TRK_KIND_VIRTUAL_DIR ||
               m->kind == TIKU_TRK_KIND_VOLUME) {
        m->facts.perm |= TIKU_TRK_P_READ;
    }
    ...
    snprintf(m->type, sizeof m->type, "%s",
             (m->kind == TIKU_TRK_KIND_FILE)      ? "tiku-file/data"
             : (m->kind == TIKU_TRK_KIND_DIRECTORY) ? "tiku-dir/store"
             : (m->kind == TIKU_TRK_KIND_VOLUME)    ? "tiku-dir/device"
 ...
 */
static int
tk_changes(tiku_trk_backend_t *b, tiku_trk_change_t *out, int max,
           int *complete)
{
    tiku_impl_t *impl = b->impl;
    tiku_desk_change_t recs[32];
    unsigned dropped = 0u;
    int n, i, w = 0;
    ...
    n = tiku_desk_ns_events(impl->ns, recs,
    ...
    for (i = 0; i < n; i++) {
        if (strcmp(recs[i].op, "changed") != 0) {
            (void)tiku_desk_ns_load(impl->ns);
            break;
        }
    }
    for (i = 0; i < n && w < max; i++) {
        ...
        if (strcmp(recs[i].op, "created") == 0) {
            out[w].op = TIKU_TRK_CH_CREATED;
        } else if (strcmp(recs[i].op, "removed") == 0) {
            out[w].op = TIKU_TRK_CH_REMOVED;
        } else if (strcmp(recs[i].op, "moved") == 0) {
            out[w].op = TIKU_TRK_CH_MOVED;
        } else {
            out[w].op = TIKU_TRK_CH_CHANGED;
        }
        w++;
    }
    return w;
...
static int
tk_path_of_id(tiku_trk_backend_t *b, uint64_t id, char *out, size_t max)
{
    tiku_impl_t *impl = b->impl;
    const tiku_desk_node_t *n = tiku_desk_ns_find_id(impl->ns,
                                                     (unsigned)id);
...
static int
tk_stat(tiku_trk_backend_t *b, const char *path, tiku_trk_model_t *out)
{
    tiku_impl_t *impl = b->impl;
    const tiku_desk_node_t *n = tiku_desk_ns_find(impl->ns, path);
    ...
    if (n == NULL) {
        ...
        int rc = tiku_desk_ns_read(impl->ns, path);
        ...
        out->backend = b;
        if (rc >= 0) {
            out->kind = TIKU_TRK_KIND_DEVICE_NODE;
            out->facts.perm = TIKU_TRK_P_READ;
        ...
        }
        {
            ...
            nls = tiku_desk_ns_ls(impl->ns, dir, names, 64);
            for (k = 0; k < nls; k++) {
                if (slash != NULL && strcmp(names[k], slash + 1) == 0) {
                    out->kind = TIKU_TRK_KIND_UNKNOWN;
                    return 0;
                }
            }
        }
...
static int
tk_list(tiku_trk_backend_t *b, const char *path, tiku_trk_entry_fn fn,
        void *ctx)
{
    tiku_impl_t *impl = b->impl;
    const tiku_desk_node_t *kids[256];
    int n, i, count;
     ...
     */
    {
        char names[128][64];
        char kinds[128];
        int live = tiku_desk_ns_ls_kinds(impl->ns, path, names, kinds, 128);

        count = 0;
        if (live > 0) {
            for (i = 0; i < live; i++) {
                tiku_trk_model_t m;
                const tiku_desk_node_t *known;
                char full[TIKU_TRK_PATH_MAX];
                ...
                known = tiku_desk_ns_find(impl->ns, full);
                if (known != NULL) {
                    from_node(b, known, &m);
                } else {
                    ...
                    (void)snprintf(m.facts.req_cap, sizeof m.facts.req_cap,
                                   "?");
                    m.kind = kinds[i] ? TIKU_TRK_KIND_VIRTUAL_DIR
                                      : TIKU_TRK_KIND_DEVICE_NODE;
                    m.facts.perm = TIKU_TRK_P_READ;
                    m.facts.cap_known = 0;
                    m.backend = b;
        ...
        n = tiku_desk_ns_children(impl->ns, path, kids, 256);
        if (n <= 0 && tiku_desk_ns_find(impl->ns, path) == NULL) {
            return -1;
        }
        for (i = 0; i < n; i++) {
            tiku_trk_model_t m;
    ...
    if (n == 0 && strcmp(path, "/data") == 0) {
        char names[64][64];
        int k = tiku_desk_ns_ls(impl->ns, path, names, 64);
...
static int
tk_read(tiku_trk_backend_t *b, const char *path, void *buf, size_t max)
{
    tiku_impl_t *impl = b->impl;
    const tiku_desk_node_t *n;
    size_t len;

    if (tiku_desk_ns_read(impl->ns, path) != 0) {
        return -1;
    }
    n = tiku_desk_ns_find(impl->ns, path);
    if (n == NULL || !n->value_valid) {
        return -1;
    }
    len = strlen(n->value);
...
static int
tk_write(tiku_trk_backend_t *b, const char *path, const void *buf, size_t len,
         char *err, size_t errmax)
{
    ...
    return tiku_desk_ns_write(impl->ns, path, val, err, errmax);
}

static int
tk_pump(tiku_trk_backend_t *b, int timeout_ms)
{
    tiku_impl_t *impl = b->impl;

    return tiku_desk_ns_pump(impl->ns, timeout_ms);
}

static tiku_trk_store_t *
tk_store(tiku_trk_backend_t *b)
{
    tiku_impl_t *impl = b->impl;

    if (impl->store == NULL) {
        impl->store = tiku_trk_store_sidecar_open(impl->devid);
    }
    return impl->store;
}

static void
tk_close(tiku_trk_backend_t *b)
{
    tiku_impl_t *impl = b->impl;
    ...
    if (impl != NULL) {
        ...
        if (impl->store != NULL) {
            (void)tiku_trk_store_flush(impl->store);
            tiku_trk_store_free(impl->store);
        }
        tiku_desk_ns_free(impl->ns);
        tiku_desk_session_free(impl->sess);
        free(impl);
    }
    free(b);
}

static const tiku_trk_backend_ops_t TIKU_OPS = {
...
tiku_trk_backend_t *
tiku_trk_backend_tiku_open(const char *port, int baud)
{
    tiku_trk_backend_t *b;
    tiku_impl_t *impl;
    tiku_desk_tx_t *tx;
    char scan[8][256];

    if (port == NULL) {
        if (tiku_desk_tx_scan_serial(scan, 8) <= 0) {
            return NULL;
        }
        port = scan[0];
    }
    tx = tiku_desk_tx_open_serial(port, baud);
    if (tx == NULL) {
        return NULL;
    }
    b = calloc(1, sizeof *b);
    ...
    if (b == NULL || impl == NULL) {
        tiku_desk_tx_close(tx);
        free(b);
        free(impl);
        return NULL;
    }
    impl->sess = tiku_desk_session_new(tx);
    if (impl->sess == NULL || tiku_desk_session_sync(impl->sess, 4000) != 0) {
        tiku_desk_session_free(impl->sess);
        free(impl);
        free(b);
        return NULL;
    }
    impl->ns = tiku_desk_ns_new(impl->sess);
    if (impl->ns == NULL || tiku_desk_ns_load(impl->ns) < 0) {
        tiku_desk_ns_free(impl->ns);
        tiku_desk_session_free(impl->sess);
        free(impl);
        free(b);
    ...
    if (tiku_desk_ns_truncated(impl->ns)) {
        (void)tiku_desk_ns_complete(impl->ns, 4);
    }
    ...
    (void)tiku_trk_ident_key(&impl->ident, impl->devid, sizeof impl->devid);
...
const tiku_trk_identity_t *
tiku_trk_backend_tiku_identity(const tiku_trk_backend_t *b)
{
