 ...
 *
 * tiku_trk_fs.c - file operations, stepped rather than blocking.
 *
 * An operation is a queue plus a cursor: tiku_trk_fs_step() does a little and
...
#include "tiku_trk_fs.h"
#include "tiku_trk_state.h"
#include "tiku_trk_volume.h"
...
typedef struct {
    char from[TIKU_TRK_PATH_MAX];   /* where it ended up */
    char to[TIKU_TRK_PATH_MAX];     /* where it came from, "" if created */
...
struct tiku_trk_fs {
    tiku_trk_backend_t *backend;
    char                queue[FS_QUEUE_MAX][TIKU_TRK_PATH_MAX];
    ...
    int                 qcount;
    int                 qcursor;
    char                dst[TIKU_TRK_PATH_MAX];
    tiku_trk_conflict_t policy;
    tiku_trk_progress_t prog;
    ...
    tiku_trk_cancel_fn  cancel_fn;
    void               *cancel_ctx;
    tiku_trk_fs_moved_fn moved_fn;
    void               *moved_ctx;
...
static void
landed(tiku_trk_fs_t *fs, const char *from, const char *to)
{
...
void
tiku_trk_fs_set_moved(tiku_trk_fs_t *fs, tiku_trk_fs_moved_fn fn, void *ctx)
{
    if (fs != NULL) {
        fs->moved_fn = fn;
        fs->moved_ctx = ctx;
...
tiku_trk_fs_t *
tiku_trk_fs_new(tiku_trk_backend_t *backend)
{
    tiku_trk_fs_t *fs = calloc(1, sizeof *fs);
...
void
tiku_trk_fs_free(tiku_trk_fs_t *fs)
{
    if (fs != NULL) {
...
int
tiku_trk_fs_busy(const tiku_trk_fs_t *fs)
{
    return (fs != NULL && fs->prog.op != TIKU_TRK_OP_IDLE &&
            fs->qcursor < fs->qcount);
}

const tiku_trk_progress_t *
tiku_trk_fs_progress(const tiku_trk_fs_t *fs)
{
...
const char *
tiku_trk_fs_name_deny(void)
{
    return NAME_DENY;
}

int
tiku_trk_fs_name_ok(const char *name, char *err, size_t errmax)
{
...
static int
exists(tiku_trk_fs_t *fs, const char *dir, const char *name)
{
...
static int
free_suffixed_name(tiku_trk_fs_t *fs, const char *dir, const char *name,
                   const char *word, int plain_first, char *out, size_t max)
{
    char base[TIKU_TRK_NAME_MAX];
    int n, i;
...
int
tiku_trk_fs_duplicate_name(tiku_trk_fs_t *fs, const char *dir,
...
int
tiku_trk_fs_link_name(tiku_trk_fs_t *fs, const char *dir, const char *name,
                      char *out, size_t max)
{
...
volume_of(tiku_trk_fs_t *fs, const char *path)
{
    struct stat st;
...
tiku_trk_op_t
tiku_trk_fs_drop_action(tiku_trk_fs_t *fs, const char *src,
                        const char *dst_dir, int force_copy)
{
    unsigned long a, b;

    if (force_copy) {
        return TIKU_TRK_OP_COPY;
    }
    a = volume_of(fs, src);
    ...
    return (a != 0 && a == b) ? TIKU_TRK_OP_MOVE : TIKU_TRK_OP_COPY;
}
 ...
 */
static void
undo_reset(tiku_trk_fs_t *fs, tiku_trk_op_t op)
{
    undo_op_t fresh;
...
static void
undo_push(tiku_trk_fs_t *fs, const char *from, const char *to, int created)
{
    undo_op_t  *o;
    undo_rec_t *grown, *r;
 ...
 */
static int
contains_trash(tiku_trk_fs_t *fs, const char *path)
{
    char probe[TIKU_TRK_PATH_MAX];
    size_t n;
...
static int
start(tiku_trk_fs_t *fs, tiku_trk_op_t op, const char *const *paths, int n,
      const char *dst, tiku_trk_conflict_t policy)
{
    int i;

    if (fs == NULL || tiku_trk_fs_busy(fs)) {
        return -1;
    }
    if (n > FS_QUEUE_MAX) {
        n = FS_QUEUE_MAX;
    ...
    fs->qcount = 0;
    for (i = 0; i < n; i++) {
        if ((op == TIKU_TRK_OP_COPY || op == TIKU_TRK_OP_MOVE) &&
            dst != NULL) {
            ...
            if (tiku_trk_fs_check_move(fs, paths[i], dst, why,
                                       sizeof why) != TIKU_TRK_REFUSE_NONE) {
        ...
        if ((op == TIKU_TRK_OP_TRASH || op == TIKU_TRK_OP_DELETE) &&
            ...
            tiku_trk_volumes_t vs;

            if (tiku_trk_volumes_scan(&vs) > 0 &&
                tiku_trk_volume_is_root(&vs, paths[i])) {
        ...
        snprintf(fs->queue[fs->qcount++], TIKU_TRK_PATH_MAX, "%s", paths[i]);
    }
    fs->qcursor = 0;
    fs->policy = policy;
    ...
    {   /* What the VOLUME allows, before a byte moves.  A move off a
        ...
        if (fs->backend->devid[0] == '\0' &&
            tiku_trk_volumes_scan(&vs) > 0) {
            if (dst != NULL &&
                !tiku_trk_volume_may_write(&vs, dst, 1, why, sizeof why)) {
                snprintf(fs->prog.error, sizeof fs->prog.error,
            ...
            if (op == TIKU_TRK_OP_MOVE) {
                for (k = 0; k < fs->qcount; k++) {
                    if (!tiku_trk_volume_may_write(&vs, fs->queue[k], 1,
                                                   why, sizeof why)) {
                        op = TIKU_TRK_OP_COPY;
                        break;
                    }
                }
            } else if (op == TIKU_TRK_OP_TRASH || op == TIKU_TRK_OP_DELETE) {
                for (k = 0; k < fs->qcount; k++) {
                    if (!tiku_trk_volume_may_write(&vs, fs->queue[k], 1,
                                                   why, sizeof why)) {
                        snprintf(fs->prog.error, sizeof fs->prog.error,
    ...
    if (op == TIKU_TRK_OP_COPY && dst != NULL &&
        !tiku_trk_fs_in_trash(fs, dst) && fs->backend->devid[0] == '\0') {
        ...
        total = tiku_trk_fs_items_size(q, fs->qcount);
        if (!tiku_trk_fs_fits(total, tiku_trk_fs_free_bytes(dst))) {
            snprintf(fs->prog.error, sizeof fs->prog.error,
...
int
tiku_trk_fs_copy(tiku_trk_fs_t *fs, const char *const *paths, int n,
                 const char *dst_dir, tiku_trk_conflict_t policy)
{
    return start(fs, TIKU_TRK_OP_COPY, paths, n, dst_dir, policy);
}

int
tiku_trk_fs_move(tiku_trk_fs_t *fs, const char *const *paths, int n,
                 const char *dst_dir, tiku_trk_conflict_t policy)
{
    return start(fs, TIKU_TRK_OP_MOVE, paths, n, dst_dir, policy);
}

int
tiku_trk_fs_duplicate(tiku_trk_fs_t *fs, const char *const *paths, int n)
{
    return start(fs, TIKU_TRK_OP_DUPLICATE, paths, n, NULL,
                 TIKU_TRK_CONFLICT_RENAME);
}

int
tiku_trk_fs_trash(tiku_trk_fs_t *fs, const char *const *paths, int n)
{
    return start(fs, TIKU_TRK_OP_TRASH, paths, n, NULL,
                 TIKU_TRK_CONFLICT_SKIP);
}
 ...
 */
uint64_t
tiku_trk_fs_free_bytes(const char *path)
{
    struct statvfs v;
...
uint64_t
tiku_trk_fs_items_size(const char *const *paths, int n)
{
    uint64_t total = 0u;
    int i;
    ...
    for (i = 0; i < n; i++) {
        ...
        if (d != NULL) {
            ...
            while ((e = readdir(d)) != NULL) {
                ...
                one[0] = child;
                total += tiku_trk_fs_items_size(one, 1);
            }
            (void)closedir(d);
        }
    }
...
int
tiku_trk_fs_fits(uint64_t total, uint64_t free_bytes)
{
...
tiku_trk_confirm_t
tiku_trk_fs_confirm_move(const char *path)
{
    ...
    if (path == NULL || path[0] == '\0') {
        return TIKU_TRK_CONFIRM_NONE;
    }
    for (i = 0; i < sizeof system_dirs / sizeof system_dirs[0]; i++) {
        if (under(path, system_dirs[i])) {
            return TIKU_TRK_CONFIRM_OVERRIDE;
        }
    }
    if (home != NULL && home[0] != '\0') {
        char cfg[TIKU_TRK_PATH_MAX];
...
int
tiku_trk_fs_in_trash(tiku_trk_fs_t *fs, const char *path)
{
    char trash[TIKU_TRK_PATH_MAX];
    size_t n;

    if (fs == NULL || path == NULL ||
        tiku_trk_fs_trash_dir(fs, path, trash, sizeof trash) != 0) {
        return 0;
    }
    n = strlen(trash);
...
int
tiku_trk_fs_link(tiku_trk_fs_t *fs, const char *const *paths, int n,
                 const char *dst, int relative)
{
    ...
    return start(fs, TIKU_TRK_OP_LINK, paths, n, dst,
                 TIKU_TRK_CONFLICT_ASK);
}

int
tiku_trk_fs_delete(tiku_trk_fs_t *fs, const char *const *paths, int n)
{
    return start(fs, TIKU_TRK_OP_DELETE, paths, n, NULL,
                 TIKU_TRK_CONFLICT_ASK);
}
/*---------------------------------------------------------------------------*/
/* the work                                                                  */
 ...
 */
static void
copy_pose_info(tiku_trk_fs_t *fs, const char *src, const char *dst)
{
    tiku_trk_store_t *store;
    ...
    if (store == NULL || !tiku_trk_store_writable(store)) {
        return;
    }
    n = tiku_trk_state_read(store, src, TIKU_TRK_ATTR_POSE_INFO, raw,
                            sizeof raw);
    if (n > 0) {
        (void)tiku_trk_state_write(store, dst, TIKU_TRK_ATTR_POSE_INFO, raw,
                                   (size_t)n);
    }
}

static int
copy_bytes(tiku_trk_fs_t *fs, const char *src, const char *dst, char *err,
           size_t errmax)
{
    ...
    {   /* Re-checked per file, not only up front: another process may
        ...
        freeb = tiku_trk_fs_free_bytes(dir);
 ...
 */
static int
copy_tree(tiku_trk_fs_t *fs, const char *src, const char *dstpath, char *err,
          size_t errmax)
{
    struct stat st;
    dev_t sdev;
 ...
 */
static int
move_tree(tiku_trk_fs_t *fs, const char *src, const char *dstpath, char *err,
          size_t errmax)
{
...
tiku_trk_refusal_t
tiku_trk_fs_check_move(tiku_trk_fs_t *fs, const char *src, const char *dst_dir,
                       char *msg, size_t max)
{
    char dstpath[TIKU_TRK_PATH_MAX];

    if (msg != NULL && max > 0u) {
        msg[0] = '\0';
    }
    if (fs == NULL || src == NULL || dst_dir == NULL) {
        return TIKU_TRK_REFUSE_NONE;
    }
    /* The Trash is not an item to be moved about; it is where items go. */
    if (strstr(src, "/" TIKU_TRK_TRASH_NAME) != NULL &&
        strcmp(leaf_of(src), TIKU_TRK_TRASH_NAME) == 0) {
        if (msg != NULL) {
            snprintf(msg, max, "You can't move or copy the trash.");
        }
        return TIKU_TRK_REFUSE_TRASH;
    }
    /*
    ...
    if (fs->backend->devid[0] == '\0') {
        ...
        if (tiku_trk_volumes_scan(&vs) > 0 &&
            tiku_trk_volume_is_root(&vs, src)) {
            if (msg != NULL) {
    ...
    if (is_dir(src) && path_contains(src, dst_dir)) {
        ...
        return TIKU_TRK_REFUSE_INTO_SELF;
    }
    ...
    }
    {
        ...
        if (stat(dstpath, &dst_st) != 0) {
            return TIKU_TRK_REFUSE_NONE;    /* no collision, no question  */
        }
        ...
        if (S_ISDIR(dst_st.st_mode) && path_contains(dstpath, src)) {
            ...
            return TIKU_TRK_REFUSE_BY_CHILD;
        }
...
int
tiku_trk_fs_has_trash(const tiku_trk_fs_t *fs)
{
...
{
    char cur[TIKU_TRK_PATH_MAX], parent[TIKU_TRK_PATH_MAX];
    struct stat st, pst;
...
int
tiku_trk_fs_trash_dirs(tiku_trk_fs_t *fs, char out[][TIKU_TRK_PATH_MAX],
                       int max)
{
    tiku_trk_volumes_t vs;

    if (fs == NULL || out == NULL || max <= 0 ||
        !tiku_trk_fs_has_trash(fs)) {
        return 0;
    }
    if (fs->backend->devid[0] != '\0' ||
        tiku_trk_volumes_scan(&vs) <= 0) {
        ...
        if (tiku_trk_fs_trash_dir(fs, NULL, out[0], TIKU_TRK_PATH_MAX) ==
                0) {
            return 1;
        }
        return 0;
    }
    return tiku_trk_fs_trash_dirs_of(fs,
               (const struct tiku_trk_volumes *)&vs, out, max);
}

int
tiku_trk_fs_prepare_trash(tiku_trk_fs_t *fs)
{
    char dirs[TIKU_TRK_VOLUME_MAX][TIKU_TRK_PATH_MAX];
    ...
    return tiku_trk_fs_trash_dirs(fs, dirs, TIKU_TRK_VOLUME_MAX);
}

int
tiku_trk_fs_trash_dirs_of(tiku_trk_fs_t *fs, const struct tiku_trk_volumes *v,
                          char out[][TIKU_TRK_PATH_MAX], int max)
{
    const tiku_trk_volumes_t *vsp = (const tiku_trk_volumes_t *)v;
    int i, n = 0;
    ...
    for (i = 0; i < vsp->n && n < max; i++) {
        ...
        if (tiku_trk_fs_trash_dir(fs, vsp->v[i].mount, here,
                                  sizeof here) != 0) {
            continue;
        }
        for (k = 0; k < n; k++) {
        ...
        if (!dupe) {
            snprintf(out[n++], TIKU_TRK_PATH_MAX, "%s", here);
        }
    }
    return n;
}
...
int
tiku_trk_fs_trash_dir(tiku_trk_fs_t *fs, const char *path, char *out,
                      size_t max)
{
    char root[TIKU_TRK_PATH_MAX];
    const char *home = getenv("HOME");
    struct stat rs, hs;

    if (fs == NULL || out == NULL || !tiku_trk_fs_has_trash(fs)) {
        return -1;
    }
...
int
tiku_trk_fs_original_path(tiku_trk_fs_t *fs, const char *path, char *out,
                          size_t max)
{
    tiku_trk_store_t *store;
    int n;
    ...
    n = tiku_trk_state_read(store, path, TIKU_TRK_ATTR_ORIGINAL, out,
                            max - 1u);
    if (n <= 0) {
        out[0] = '\0';
        return 0;
...
static void
record_origin(tiku_trk_fs_t *fs, const char *trashed, const char *origin)
{
    tiku_trk_store_t *store;
    ...
    (void)tiku_trk_state_write(store, trashed, TIKU_TRK_ATTR_ORIGINAL,
                               origin, strlen(origin));
    (void)tiku_trk_store_flush(store);
}

tiku_trk_delete_q_t
tiku_trk_fs_delete_question(const tiku_trk_fs_t *fs, const char *path,
                            int shift_held)
{
    if (fs == NULL) {
        return TIKU_TRK_DELETE_STRAIGHT;
    }
    if (shift_held) {
        return TIKU_TRK_DELETE_STRAIGHT;    /* the user has decided       */
    }
    {
        ...
        if (tiku_trk_fs_trash_dir((tiku_trk_fs_t *)fs, path, trash,
                                  sizeof trash) != 0) {
            return TIKU_TRK_DELETE_STRAIGHT;
        }
    }
    if (path != NULL && strstr(path, "/" TIKU_TRK_TRASH_NAME) != NULL) {
        /* Inside the Trash the only thing left IS deletion, so asking
         * whether to move it to the Trash asks nothing (FS-047). */
        return TIKU_TRK_DELETE_STRAIGHT;
    }
    return TIKU_TRK_DELETE_ASK;
}

int
tiku_trk_fs_restore(tiku_trk_fs_t *fs, const char *const *paths, int n,
                    char *err, size_t errmax)
{
    ...
    for (i = 0; i < n; i++) {
        char origin[TIKU_TRK_PATH_MAX];
        struct stat st;

        if (!tiku_trk_fs_original_path(fs, paths[i], origin, sizeof origin)) {
            if (err != NULL) {
                snprintf(err, errmax,
...
int
tiku_trk_fs_empty_trash(tiku_trk_fs_t *fs)
{
    char dirs[TIKU_TRK_VOLUME_MAX][TIKU_TRK_PATH_MAX];
    char paths[FS_QUEUE_MAX][TIKU_TRK_PATH_MAX];
    ...
    nd = tiku_trk_fs_trash_dirs(fs, dirs, TIKU_TRK_VOLUME_MAX);
    for (k = 0; k < nd; k++) {
    ...
    (void)tiku_trk_fs_delete(fs, sel, n);
    return n;
}
 ...
 */
static int
fail_ask(tiku_trk_fs_t *fs, const char *msg, int can_continue)
{
...
static int
step_one(tiku_trk_fs_t *fs)
{
    const char *src = fs->queue[fs->qcursor];
    char dstdir[TIKU_TRK_PATH_MAX], dstpath[TIKU_TRK_PATH_MAX];
    ...
    if (fs->prog.op == TIKU_TRK_OP_TRASH && tiku_trk_fs_has_trash(fs) &&
        strstr(src, "/" TIKU_TRK_TRASH_NAME) == NULL) {
        ...
        char trash[TIKU_TRK_PATH_MAX], want[TIKU_TRK_NAME_MAX];

        if (tiku_trk_fs_trash_dir(fs, src, trash, sizeof trash) != 0) {
        ...
        if (exists(fs, trash, leaf_of(src))) {
            (void)tiku_trk_fs_duplicate_name(fs, trash, leaf_of(src), want,
                                             sizeof want);
        } else {
        ...
        if (fs->backend->devid[0] != '\0') {
            ...
            if (d != NULL) {
                ...
                while ((e = readdir(d)) != NULL &&
                    ...
                    if (join(fs->queue[fs->qcount], TIKU_TRK_PATH_MAX,
                             src, e->d_name) == 0) {
                        fs->qexpanded[fs->qcount] = 0;
                        fs->qcount++;
                    }
            ...
            if (fs->qcount > before && fs->qcount < FS_QUEUE_MAX) {
                ...
                snprintf(fs->queue[fs->qcount], TIKU_TRK_PATH_MAX, "%s",
                         src);
                fs->qcount++;
                fs->prog.total = fs->qcount;
                fs->qcursor++;
...
delete_attempt:
            if (remove(src) != 0) {
                if (errno == ENOTEMPTY || errno == EEXIST) {
                    ...
                    if (k < fs->qcount && fs->qcount < FS_QUEUE_MAX) {
                        fs->qexpanded[fs->qcount] = 1;
                        snprintf(fs->queue[fs->qcount], TIKU_TRK_PATH_MAX,
                                 "%s", src);
                        fs->qcount++;
                        fs->prog.total = fs->qcount;
                        fs->qcursor++;
    ...
    if (fs->prog.op != TIKU_TRK_OP_DUPLICATE && exists(fs, dstdir, name)) {
        switch (fs->policy) {
        case TIKU_TRK_CONFLICT_SKIP:
            ...
            fs->qcursor++;
            return 1;
        case TIKU_TRK_CONFLICT_RENAME:
            if (tiku_trk_fs_duplicate_name(fs, dstdir, name, name,
                                           sizeof name) != 0 ||
            ...
            break;
        case TIKU_TRK_CONFLICT_REPLACE:
            if (fs->pending_ask) {
                /* A one-shot "Replace": used up by this item. */
                fs->pending_ask = 0;
                fs->policy = TIKU_TRK_CONFLICT_ASK;
            }
    ...
    if (fs->prog.op == TIKU_TRK_OP_MOVE) {
...
int
tiku_trk_fs_step(tiku_trk_fs_t *fs, int budget)
{
    int n = 0;
    ...
    if (fs->qcursor >= fs->qcount && !fs->prog.conflict) {
        fs->prog.op = (fs->prog.total > 0) ? fs->prog.op : TIKU_TRK_OP_IDLE;
    }
    return n;
}

void
tiku_trk_fs_resolve(tiku_trk_fs_t *fs, tiku_trk_conflict_t how)
{
    ...
    switch (how) {
    case TIKU_TRK_CONFLICT_SKIP_ALL:
        fs->policy = TIKU_TRK_CONFLICT_SKIP;
        break;
    case TIKU_TRK_CONFLICT_REPLACE_ALL:
        fs->policy = TIKU_TRK_CONFLICT_REPLACE;
        break;
    case TIKU_TRK_CONFLICT_SKIP:
        fs->prog.skipped++;
        fs->qcursor++;
...
void
tiku_trk_fs_cancel(tiku_trk_fs_t *fs)
{
    if (fs != NULL) {
...
void
tiku_trk_fs_set_cancel_check(tiku_trk_fs_t *fs, tiku_trk_cancel_fn fn,
                             void *ctx)
{
    if (fs != NULL) {
        fs->cancel_fn = fn;
...
void
tiku_trk_fs_resolve_error(tiku_trk_fs_t *fs, int keep_going)
{
    ...
    fs->prog.failed++;
    fs->qcursor++;
    if (fs->prog.op == TIKU_TRK_OP_DELETE) {
...
int
tiku_trk_fs_rename(tiku_trk_fs_t *fs, const char *path, const char *new_name,
                   char *err, size_t errmax)
{
    char dir[TIKU_TRK_PATH_MAX], dst[TIKU_TRK_PATH_MAX];

    if (fs == NULL || path == NULL || new_name == NULL) {
        return -1;
    }
    if (!tiku_trk_fs_name_ok(new_name, err, errmax)) {
        return -1;
    }
    if (fs->backend->devid[0] == '\0') {
        tiku_trk_volumes_t vs;
        ...
        if (tiku_trk_volumes_scan(&vs) > 0) {
            ...
            if (tiku_trk_volume_is_root(&vs, path)) {
                if (err != NULL) {
                    snprintf(err, errmax,
            ...
            if (!tiku_trk_volume_may_write(&vs, path, 1, why, sizeof why)) {
                if (err != NULL) {
    ...
    }
    {
        ...
        if (fs->backend->ops->stat(fs->backend, path, &m) == 0 &&
            (m.kind == TIKU_TRK_KIND_TRASH ||
             m.kind == TIKU_TRK_KIND_DESKTOP)) {
            if (err != NULL) {
                snprintf(err, errmax,
                         "The %s cannot be renamed: it is found by its "
                         "name.", (m.kind == TIKU_TRK_KIND_TRASH)
                                      ? "Trash" : "Desktop");
            }
            return -1;
        }
    ...
    undo_reset(fs, TIKU_TRK_OP_RENAME);
...
void
tiku_trk_fs_new_folder_name(tiku_trk_fs_t *fs, const char *dir, char *out,
                            size_t max)
{
    long n;
...
int
tiku_trk_fs_new_folder(tiku_trk_fs_t *fs, const char *dir, char *out,
                       size_t max, char *err, size_t errmax)
{
    char name[TIKU_TRK_PATH_MAX], path[TIKU_TRK_PATH_MAX];

    if (fs == NULL || dir == NULL) {
        return -1;
    }
    tiku_trk_fs_new_folder_name(fs, dir, name, sizeof name);
    ...
    undo_reset(fs, TIKU_TRK_OP_NEW_FOLDER);
...
int
tiku_trk_fs_new_from_template(tiku_trk_fs_t *fs, const char *template_path,
                              const char *dst_dir, char *out, size_t max,
                              char *err, size_t errmax)
{
    struct stat st;
    char name[TIKU_TRK_NAME_MAX];
    char candidate[TIKU_TRK_NAME_MAX];
    char dst[TIKU_TRK_PATH_MAX];
    const char *base;
    int n;
    ...
    undo_reset(fs, TIKU_TRK_OP_COPY);
...
int
tiku_trk_fs_can_undo(const tiku_trk_fs_t *fs, char *what, size_t max)
{
    const undo_op_t *o;
    ...
    if (o->op == TIKU_TRK_OP_DELETE) {
        return 0;
    }
    if (what != NULL) {
        snprintf(what, max, "%s", tiku_trk_fs_op_name(o->op));
    }
    return 1;
}

int
tiku_trk_fs_last_created(const tiku_trk_fs_t *fs,
                         char out[][TIKU_TRK_PATH_MAX], int max)
{
    const undo_op_t *o;
    int i, n = 0;
    ...
    for (i = 0; i < o->n && n < max; i++) {
        ...
        if (o->rec[i].created || o->rec[i].to[0] != '\0') {
            snprintf(out[n], TIKU_TRK_PATH_MAX, "%s", o->rec[i].from);
            n++;
        }
    }
    return n;
...
int
tiku_trk_fs_can_redo(const tiku_trk_fs_t *fs, char *what, size_t max)
{
    const undo_op_t *o;
...
const char *
tiku_trk_fs_op_name(tiku_trk_op_t op)
{
 ...
 */
static int
undo_one(tiku_trk_fs_t *fs, const undo_rec_t *r, char *err, size_t errmax)
{
    if (r->created) {
 ...
 */
static int
redo_one(tiku_trk_fs_t *fs, tiku_trk_op_t op, const undo_rec_t *r, char *err,
         size_t errmax)
{
    if (op == TIKU_TRK_OP_NEW_FOLDER) {
...
int
tiku_trk_fs_undo(tiku_trk_fs_t *fs, char *err, size_t errmax)
{
    undo_op_t *o;
    int i, n = 0;

    if (!tiku_trk_fs_can_undo(fs, NULL, 0)) {
        return -1;
    }
    o = &fs->ustack[fs->ucount - 1];
...
int
tiku_trk_fs_redo(tiku_trk_fs_t *fs, char *err, size_t errmax)
{
    undo_op_t *o;
    int i, n = 0;

    if (!tiku_trk_fs_can_redo(fs, NULL, 0)) {
        return -1;
    }
    o = &fs->rstack[fs->rcount - 1];
    for (i = 0; i < o->n; i++) {
