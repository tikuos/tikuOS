 ...
 *
 * tiku_trk_attr.c - typed descriptors, formatting and validation.
 *
...
int
tiku_trk_desc_parse(const char *meta, tiku_trk_desc_t *out)
{
    char tok[24];
    const char *p;
    ...
    if (strcmp(tok, "bool") == 0)      { out->vtype = TIKU_TRK_VT_BOOL; }
    else if (strcmp(tok, "u32") == 0)  { out->vtype = TIKU_TRK_VT_U32; }
    else if (strcmp(tok, "i32") == 0)  { out->vtype = TIKU_TRK_VT_I32; }
    else if (strcmp(tok, "str") == 0)  { out->vtype = TIKU_TRK_VT_STR; }
    else                               { return 0; }

    p = field(p, out->unit_name, sizeof out->unit_name);
    if (strcmp(out->unit_name, "bool") == 0)       { out->unit = TIKU_TRK_UNIT_BOOL; }
    else if (strcmp(out->unit_name, "bytes") == 0) { out->unit = TIKU_TRK_UNIT_BYTES; }
    else if (strcmp(out->unit_name, "Hz") == 0)    { out->unit = TIKU_TRK_UNIT_HZ; }
    else if (strcmp(out->unit_name, "s") == 0)     { out->unit = TIKU_TRK_UNIT_SECONDS; }
    else if (strcmp(out->unit_name, "count") == 0) { out->unit = TIKU_TRK_UNIT_COUNT; }
    else if (out->unit_name[0] == '\0' ||
             strcmp(out->unit_name, "-") == 0)     { out->unit = TIKU_TRK_UNIT_NONE; }
    else                                           { out->unit = TIKU_TRK_UNIT_OTHER; }

    p = field(p, tok, sizeof tok);
    if (strcmp(tok, "static") == 0)      { out->fresh = TIKU_TRK_FRESH_STATIC; }
    else if (strcmp(tok, "cached") == 0) { out->fresh = TIKU_TRK_FRESH_CACHED; }
    else if (strcmp(tok, "live") == 0)   { out->fresh = TIKU_TRK_FRESH_LIVE; }
...
int
tiku_trk_attr_format(const tiku_trk_desc_t *d, const char *raw,
                     char *out, size_t max)
{
    long v;
    ...
    if (d == NULL || d->vtype == TIKU_TRK_VT_NONE ||
        d->vtype == TIKU_TRK_VT_STR || !as_long(raw, &v)) {
        snprintf(out, max, "%s", raw);
        return (int)strlen(out);
    }
    switch (d->unit) {
    case TIKU_TRK_UNIT_BOOL:
        snprintf(out, max, "%s", (v != 0) ? "on" : "off");
        break;
    case TIKU_TRK_UNIT_BYTES:
        fmt_bytes(v, out, max);
        break;
    case TIKU_TRK_UNIT_HZ:
        fmt_hz(v, out, max);
        break;
    case TIKU_TRK_UNIT_SECONDS:
        fmt_seconds(v, out, max);
        break;
    case TIKU_TRK_UNIT_COUNT:
        fmt_count(v, out, max);
        break;
    default:
        if (d->vtype == TIKU_TRK_VT_BOOL) {
            snprintf(out, max, "%s", (v != 0) ? "on" : "off");
...
int
tiku_trk_attr_editable(const tiku_trk_model_t *m, const tiku_trk_desc_t *d)
{
    (void)d;
    if (m == NULL) {
        return 0;
    ...
    return tiku_trk_model_is_writable(m);
}
...
int
tiku_trk_attr_validate(const tiku_trk_desc_t *d, const char *text, char *err,
                       size_t errmax)
{
    long v;
    ...
    if (text == NULL || text[0] == '\0') {
        ...
        if (d != NULL && d->vtype == TIKU_TRK_VT_BOOL) {
            return 1;
        }
    ...
    if (d->vtype == TIKU_TRK_VT_U32 && v < 0) {
...
int
tiku_trk_attr_canonical(const tiku_trk_desc_t *d, const char *text, char *out,
                        size_t max)
{
    if (out == NULL || max == 0u) {
        return 0;
    ...
    if (d != NULL && (d->vtype == TIKU_TRK_VT_U32 ||
                      d->vtype == TIKU_TRK_VT_I32)) {
        long v;
...
int
tiku_trk_attr_size_fit(int64_t bytes, int width,
                       ...
                       char *out, size_t max)
{
    ...
    if (measure != NULL && measure(out, ctx) > width) {
        ...
        if (measure(plain, ctx) <= width) {
            ...
            return tiku_trk_text_fit(out, width, TIKU_TRK_TRUNC_END, measure,
                                     ctx, out, max);
        }
    }
    return (int)strlen(out);
...
int
tiku_trk_text_fit(const char *in, int width, tiku_trk_trunc_t how,
...
int
tiku_trk_attr_compare(const tiku_trk_desc_t *d, const char *a, const char *b)
{
    long x, y;
    ...
    if (d != NULL && d->vtype != TIKU_TRK_VT_NONE &&
        d->vtype != TIKU_TRK_VT_STR && as_long(a, &x) && as_long(b, &y)) {
...
int
tiku_trk_attr_time_fit(int64_t when, int width,
                       int (*measure)(const char *, void *), void *ctx,
                       char *out, size_t max)
{
    ...
    {   /* Nothing fit: the date on its own, and failing that, elided. */
        ...
        return tiku_trk_text_fit(date, width, TIKU_TRK_TRUNC_MIDDLE, measure,
                                 ctx, out, max);
    }
}
...
int
tiku_trk_attr_rating(long value, char *out, size_t max)
{
    return gauge_cells(value, 10, 5, out, max);
}

int
tiku_trk_attr_gauge(const tiku_trk_desc_t *d, const char *raw, int cells,
                    char *out, size_t max)
{
    long v;
...
int
tiku_trk_attr_disambiguate(const char *path, const char *const *others,
