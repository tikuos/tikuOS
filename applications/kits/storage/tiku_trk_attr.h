 ...
 *
 * tiku_trk_attr.h - attributes, and the text a column shows for one.
 *
 ...
 */
#ifndef TIKU_TRK_ATTR_H_
#define TIKU_TRK_ATTR_H_
...
typedef enum {
    TIKU_TRK_VT_NONE = 0,        /* untyped: the text is the value        */
    TIKU_TRK_VT_BOOL,
    TIKU_TRK_VT_U32,
    TIKU_TRK_VT_I32,
    TIKU_TRK_VT_STR
...
typedef enum {
    TIKU_TRK_UNIT_NONE = 0,
    TIKU_TRK_UNIT_BOOL,
    TIKU_TRK_UNIT_BYTES,
    TIKU_TRK_UNIT_HZ,
    TIKU_TRK_UNIT_SECONDS,
    TIKU_TRK_UNIT_COUNT,
    TIKU_TRK_UNIT_OTHER          /* known name, no formatter of its own   */
} tiku_trk_unit_t;

/** @brief How current a reading is, which says whether it needs re-reading. */
typedef enum {
    TIKU_TRK_FRESH_UNKNOWN = 0,
    TIKU_TRK_FRESH_STATIC,       /* fixed for the life of the boot        */
    TIKU_TRK_FRESH_CACHED,
    TIKU_TRK_FRESH_LIVE
} tiku_trk_fresh_t;

#define TIKU_TRK_UNIT_NAME_MAX 12

/** @brief A parsed typed descriptor. */
typedef struct {
    tiku_trk_vtype_t vtype;
    tiku_trk_unit_t  unit;
    tiku_trk_fresh_t fresh;
    char             unit_name[TIKU_TRK_UNIT_NAME_MAX];  /* verbatim      */
    char             cost[TIKU_TRK_UNIT_NAME_MAX];
    int              has_range;
    long             lo, hi;
} tiku_trk_desc_t;
 ...
 */
int tiku_trk_desc_parse(const char *meta, tiku_trk_desc_t *out);
 ...
 */
int tiku_trk_attr_format(const tiku_trk_desc_t *d, const char *raw,
                         char *out, size_t max);
 ...
 */
int tiku_trk_attr_editable(const tiku_trk_model_t *m,
                           const tiku_trk_desc_t *d);
 ...
 */
int tiku_trk_attr_validate(const tiku_trk_desc_t *d, const char *text,
 ...
 */
int tiku_trk_attr_canonical(const tiku_trk_desc_t *d, const char *text,
                            char *out, size_t max);
 ...
 * @return <0, 0 or >0.
 */
int tiku_trk_attr_compare(const tiku_trk_desc_t *d, const char *a,
                          const char *b);

/** @brief Where an over-long string loses its middle, or its end. */
typedef enum {
    TIKU_TRK_TRUNC_MIDDLE = 0,   /* names and generic values              */
    TIKU_TRK_TRUNC_END,          /* sizes, and the open-with relation     */
    TIKU_TRK_TRUNC_BEGIN         /* type-ahead: the newest characters win */
} tiku_trk_trunc_t;
 ...
 * @return Length written.
 */
int tiku_trk_text_fit(const char *in, int width, tiku_trk_trunc_t how,
 ...
 * @return Length written.
 */
int tiku_trk_attr_time_fit(int64_t when, int width,
                           int (*measure)(const char *, void *), void *ctx,
                           char *out, size_t max);
 ...
 */
int tiku_trk_attr_size_fit(int64_t bytes, int width,
                           int (*measure)(const char *, void *), void *ctx,
                           char *out, size_t max);
 ...
 */
int tiku_trk_attr_rating(long value, char *out, size_t max);
 ...
 */
int tiku_trk_attr_gauge(const tiku_trk_desc_t *d, const char *raw, int cells,
                        char *out, size_t max);
 ...
 */
int tiku_trk_attr_disambiguate(const char *path,
                               const char *const *others, int n,
                               char *out, size_t max);

#endif /* TIKU_TRK_ATTR_H_ */
