 ...
 *
 * tiku_trk_state.c - the three state stores behind one interface.
 *
...
static state_rec_t *
rec_find(tiku_trk_store_t *s, const char *node, const char *attr)
{
    state_rec_t *r;
...
static int
rec_set(tiku_trk_store_t *s, const char *node, const char *attr,
...
static int
sidecar_load(tiku_trk_store_t *s)
{
...
static int
sidecar_save(tiku_trk_store_t *s)
{
    char tmp[540];
    FILE *f;
    state_rec_t *r;
...
{
    tiku_trk_store_t *s = calloc(1, sizeof *s);
...
tiku_trk_store_t *
tiku_trk_store_xattr_open(void)
{
    return store_new(STORE_XATTR);
}

tiku_trk_store_t *
tiku_trk_store_memory_open(void)
{
    return store_new(STORE_MEMORY);
}

tiku_trk_store_t *
tiku_trk_store_sidecar_open(const char *devid)
{
    tiku_trk_store_t *s;
    char dir[512];
...
void
tiku_trk_store_free(tiku_trk_store_t *s)
{
    state_rec_t *r;
...
int
tiku_trk_store_writable(const tiku_trk_store_t *s)
{
...
int
tiku_trk_state_read(tiku_trk_store_t *s, const char *node, const char *attr,
                    void *buf, size_t max)
{
...
int
tiku_trk_state_write(tiku_trk_store_t *s, const char *node, const char *attr,
                     const void *buf, size_t len)
{
    if (s == NULL || node == NULL || attr == NULL || buf == NULL) {
        return -1;
...
int
tiku_trk_state_each(tiku_trk_store_t *s, const char *node,
...
int
tiku_trk_state_forget(tiku_trk_store_t *s, const char *node)
{
    state_rec_t **p;
    int n = 0;
...
int
tiku_trk_store_flush(tiku_trk_store_t *s)
{
    if (s == NULL) {
        return -1;
    }
...
int
tiku_trk_store_count(const tiku_trk_store_t *s)
{
    const state_rec_t *r;
    int n = 0;
...
const char *
tiku_trk_desktop_dir(char *out, size_t max)
{
    const char *set = getenv("TIKU_DESK_DESKTOP");
    const char *home = getenv("HOME");
...
const char *
tiku_trk_fonts_dir(char *out, size_t max)
{
    const char *set = getenv("TIKU_DESK_FONTS");
    const char *home = getenv("HOME");
...
int
tiku_trk_state_mkparents(const char *path)
{
    char dir[1024];
    char *slash;
    size_t i;
