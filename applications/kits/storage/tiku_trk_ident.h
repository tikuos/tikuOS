 ...
 *
 * tiku_trk_ident.h - the one device identity, keyed on by everything durable.
 *
 ...
 */
#ifndef TIKU_TRK_IDENT_H_
#define TIKU_TRK_IDENT_H_

#include <stddef.h>

#define TIKU_TRK_DEVID_MAX 40
...
#define TIKU_TRK_W_UID   8
#define TIKU_TRK_W_ID    4
#define TIKU_TRK_W_MCU   2
#define TIKU_TRK_W_NAME  1
#define TIKU_TRK_BIND    7
 ...
 * tiku_trk_ident_provisional()).
 *
 * @return Bytes written, excluding the terminator.
 */
int tiku_trk_ident_key(const tiku_trk_identity_t *id, char *out, size_t max);

/** @brief Whether the key is a guess rather than a die identifier. */
int tiku_trk_ident_provisional(const tiku_trk_identity_t *id);
 ...
 */
int tiku_trk_ident_score(const tiku_trk_identity_t *known,
                         const tiku_trk_identity_t *seen);

/** @brief Whether @p seen should be treated as @p known. */
int tiku_trk_ident_matches(const tiku_trk_identity_t *known,
                           const tiku_trk_identity_t *seen);

#endif /* TIKU_TRK_IDENT_H_ */
