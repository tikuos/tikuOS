 ...
 *
 * tiku_trk_vdir.c - merging several real directories into one, by priority.
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include "tiku_trk_vdir.h"
...
void
tiku_trk_vdir_init(tiku_trk_vdir_t *v)
{
    if (v != NULL) {
        memset(v, 0, sizeof *v);
    }
...
int
tiku_trk_vdir_add(tiku_trk_vdir_t *v, const char *path)
{
    if (v == NULL || path == NULL || path[0] == '\0' ||
        v->nsrc >= TIKU_TRK_VDIR_MAX_SRC) {
        return -1;
    }
...
int
tiku_trk_vdir_load(tiku_trk_vdir_t *v, const char *deffile)
{
    FILE *f;
    char line[TIKU_TRK_PATH_MAX];
...
int
tiku_trk_vdir_owner(const tiku_trk_vdir_t *v, tiku_trk_backend_t *b,
                    const char *name)
{
    int i;
...
int
tiku_trk_vdir_resolve(const tiku_trk_vdir_t *v, tiku_trk_backend_t *b,
                      const char *name, char *out, size_t max)
{
    int i = tiku_trk_vdir_owner(v, b, name);
...
int
tiku_trk_vdir_after_change(const tiku_trk_vdir_t *v, tiku_trk_backend_t *b,
                           ...
                           char *out, size_t max)
{
    ...
    out[0] = '\0';
    owner = tiku_trk_vdir_owner(v, b, name);
    if (created) {
...
static int
merge_one(const tiku_trk_model_t *m, void *ctx)
{
...
int
tiku_trk_vdir_list(const tiku_trk_vdir_t *v, tiku_trk_backend_t *b,
...
void
tiku_trk_vdir_child(const tiku_trk_vdir_t *v, tiku_trk_backend_t *b,
                    const char *name, tiku_trk_vdir_t *out)
{
    ...
    for (i = 0; i < v->nsrc; i++) {
        ...
        if (join(path, sizeof path, v->src[i], name) == 0 &&
            ...
            (void)tiku_trk_vdir_add(out, path);
        }
    }
}
...
void
tiku_trk_vdir_watch_init(tiku_trk_vdir_watch_t *w, const tiku_trk_vdir_t *v)
{
    int i;
...
unsigned
tiku_trk_vdir_poll(const tiku_trk_vdir_t *v, tiku_trk_vdir_watch_t *w,
                   int *gone)
{
    unsigned ch = TIKU_TRK_VDIR_CH_NONE;
    int i;
    ...
    if (v->def[0] != '\0') {
        ...
        if (t != w->def_mtime) {
            w->def_mtime = t;
            ch |= TIKU_TRK_VDIR_CH_DEF;
        }
    }
    for (i = 0; i < v->nsrc && i < TIKU_TRK_VDIR_MAX_SRC; i++) {
        ...
        if (w->src_present[i] && !here) {
            ...
            ch |= TIKU_TRK_VDIR_CH_GONE;
            if (gone != NULL && *gone < 0) {
                *gone = i;
            }
        } else if (here && t != w->src_mtime[i]) {
            ch |= TIKU_TRK_VDIR_CH_SRC;
        }
        w->src_mtime[i] = t;
...
int
tiku_trk_vdir_supplied_by(const tiku_trk_vdir_t *v, tiku_trk_backend_t *b,
                          const char *name, int which)
{
    return (tiku_trk_vdir_owner(v, b, name) == which);
}

void
tiku_trk_vdir_drop(tiku_trk_vdir_t *v, int which)
{
    int i;
...
int
tiku_trk_vdir_cleanup(tiku_trk_vdir_t *v, const char *scratch_dir)
{
    int removed = 0;
...
uint64_t
tiku_trk_vdir_child_id(const tiku_trk_vdir_t *v, const char *name)
{
