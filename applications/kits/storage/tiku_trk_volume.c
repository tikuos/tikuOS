 ...
 *
 * tiku_trk_volume.c - reading the mount table and answering for it.
 *
...
int
tiku_trk_volumes_scan(tiku_trk_volumes_t *vs)
{
    FILE *f;
    char line[1024];
    ...
    while (fgets(line, sizeof line, f) != NULL &&
           vs->n < TIKU_TRK_VOLUME_MAX) {
        char dev[256], mount[TIKU_TRK_PATH_MAX], type[64], opts[256];
        tiku_trk_volume_t *v;
        struct stat st;
        struct statvfs sv;
        ...
        v->shared = tiku_trk_volume_shared_type(type);
...
int
tiku_trk_volume_shared_type(const char *fstype)
{
    if (fstype == NULL) {
        return 0;
    }
...
unsigned
tiku_trk_volume_flags(const tiku_trk_volume_t *v)
{
    unsigned f = 0u;
    ...
    if (v->read_only) { f |= TIKU_TRK_VOL_READ_ONLY; }
    if (v->is_boot)   { f |= TIKU_TRK_VOL_BOOT; }
    if (v->removable) { f |= TIKU_TRK_VOL_REMOVABLE; }
    if (v->shared)    { f |= TIKU_TRK_VOL_SHARED; }
    return f;
}

const tiku_trk_volume_t *
tiku_trk_volume_of(const tiku_trk_volumes_t *vs, const char *path)
{
    const tiku_trk_volume_t *best = NULL;
    struct stat st;
    int i;
...
int
tiku_trk_volume_is_root(const tiku_trk_volumes_t *vs, const char *path)
{
    int i;
...
int
tiku_trk_volume_may_unmount(const tiku_trk_volumes_t *vs, const char *path,
                            char *why, size_t max)
{
...
int
tiku_trk_volume_may_write(const tiku_trk_volumes_t *vs, const char *path,
...
int
tiku_trk_volume_shows_icon(const tiku_trk_volumes_t *vs,
                           const tiku_trk_volume_t *v)
{
    size_t n;
    int i;
...
tiku_trk_space_t
tiku_trk_volume_space(const tiku_trk_volume_t *v, int *used_pct)
{
    if (v == NULL) {
        if (used_pct != NULL) { *used_pct = 0; }
        return TIKU_TRK_SPACE_FREE;
    }
    return tiku_trk_volume_space_facts(v->total, v->avail, used_pct);
}

tiku_trk_space_t
tiku_trk_volume_space_facts(uint64_t total, uint64_t avail, int *used_pct)
{
    ...
    if (avail * 100u < total * (uint64_t)TIKU_TRK_VOLUME_WARN_PCT) {
        return TIKU_TRK_SPACE_WARNING;
    }
    return TIKU_TRK_SPACE_USED;
}
...
static int
same_volume(const tiku_trk_volume_t *a, const tiku_trk_volume_t *b)
{
...
void
tiku_trk_volwatch_init(tiku_trk_volwatch_t *w)
{
    if (w == NULL) {
        return;
    }
    memset(w, 0, sizeof *w);
    if (tiku_trk_volumes_scan(&w->was) >= 0) {
        w->primed = 1;
    }
}

int
tiku_trk_volwatch_diff(const tiku_trk_volumes_t *was,
...
int
tiku_trk_volwatch_poll(tiku_trk_volwatch_t *w, tiku_trk_volchange_t *out,
                       int max)
{
    tiku_trk_volumes_t now;
    int n;
    ...
    if (!w->primed) {
        tiku_trk_volwatch_init(w);
        return 0;               /* the first look is the baseline */
    }
    if (tiku_trk_volumes_scan(&now) < 0) {
        return 0;
    }
    n = tiku_trk_volwatch_diff(&w->was, &now, out, max);
...
int
tiku_trk_partitions_scan(tiku_trk_partition_t *out, int max)
{
    ...
    have_vols = tiku_trk_volumes_scan(&vs) > 0;
...
int
tiku_trk_volume_mount(const char *devname, char *why, size_t max)
{
    char dev[80];
...
tiku_trk_unmount_t
tiku_trk_volume_unmount(const tiku_trk_volumes_t *vs, const char *path,
                        tiku_trk_before_unmount_fn before, void *ctx,
                        char *why, size_t max)
{
    const tiku_trk_volume_t *v;
    ...
    if (!tiku_trk_volume_may_unmount(vs, path, why, max)) {
        return TIKU_TRK_UNMOUNT_REFUSED;
    }
    v = tiku_trk_volume_of(vs, path);
    if (v == NULL) {
        return TIKU_TRK_UNMOUNT_REFUSED;
    ...
    if (umount(v->mount) != 0) {
        ...
        return (errno == EBUSY) ? TIKU_TRK_UNMOUNT_BUSY
                                : TIKU_TRK_UNMOUNT_FAILED;
    }
    return TIKU_TRK_UNMOUNT_OK;
}
