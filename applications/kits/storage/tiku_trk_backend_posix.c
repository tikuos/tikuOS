 ...
 *
 * tiku_trk_backend_posix.c - local files, at full Tracker fidelity.
 *
...
static tiku_trk_store_t *px_store(tiku_trk_backend_t *b);
 ...
 */
static int
volume_here(const char *path, tiku_trk_model_t *m)
{
    static tiku_trk_volumes_t vs;
    static time_t seen;
    time_t now = time(NULL);
    int i;

    if (now != seen) {
        (void)tiku_trk_volumes_scan(&vs);
        seen = now;
    }
    for (i = 0; i < vs.n; i++) {
        if (strcmp(vs.v[i].mount, path) == 0) {
            ...
            m->facts.vol_flags = tiku_trk_volume_flags(&vs.v[i]);
...
{
    char desk[TIKU_TRK_PATH_MAX];

    return strcmp(path, tiku_trk_desktop_dir(desk, sizeof desk)) == 0;
}
...
static void
from_stat(tiku_trk_model_t *m, const char *path, const struct stat *st,
          int is_link)
{
    ...
    if (is_link) {
        ...
        m->kind = (strcmp(path, "/") == 0) ? TIKU_TRK_KIND_ROOT
                                             : TIKU_TRK_KIND_VOLUME;
    } else if (S_ISDIR(st->st_mode)) {
        /* A directory called ".Trash" IS the Trash, recognised by name the
         * way a folder called "Desktop" is (tiku_trk_model.c).  The store
         * has no other way to know: the Trash is a convention about a
         * location, not a property of the inode. */
        m->kind = (strcmp(m->name, ".Trash") == 0) ? TIKU_TRK_KIND_TRASH
                : desktop_here(path) ? TIKU_TRK_KIND_DESKTOP
                                     : TIKU_TRK_KIND_DIRECTORY;
    ...
    if (access(path, R_OK) == 0) { m->facts.perm |= TIKU_TRK_P_READ; }
    if (access(path, W_OK) == 0) { m->facts.perm |= TIKU_TRK_P_WRITE; }
    if (access(path, X_OK) == 0) { m->facts.perm |= TIKU_TRK_P_EXEC; }
    snprintf(m->type, sizeof m->type, "%s",
             m->kind == TIKU_TRK_KIND_ROOT ? "application/x-vnd.Be-root"
             : S_ISDIR(st->st_mode) ? "application/x-vnd.Be-directory"
                                  : type_by_name(m->name));
}

static int
px_stat(tiku_trk_backend_t *b, const char *path, tiku_trk_model_t *out)
{
    struct stat st, lst;
    ...
    if (out->kind == TIKU_TRK_KIND_DESKTOP && impl != NULL) {
        tiku_trk_store_t *store = px_store(b);
        char existing[16];
        ...
        if (tiku_trk_state_read(store, path, "_trk/icon", existing,
                                sizeof existing) >= 0) {
            out->backend = b;
            return 0;
        }
        ...
        (void)tiku_trk_state_write(store, path, "_trk/icon",
...
static int
px_list(tiku_trk_backend_t *b, const char *path, tiku_trk_entry_fn fn,
        void *ctx)
{
    DIR *d = opendir(path);
    struct dirent *e;
...
static int
px_read(tiku_trk_backend_t *b, const char *path, void *buf, size_t max)
{
...
static int
px_write(tiku_trk_backend_t *b, const char *path, const void *buf, size_t len,
...
static int
px_pump(tiku_trk_backend_t *b, int timeout_ms)
{
...
px_store(tiku_trk_backend_t *b)
{
    posix_impl_t *impl = b->impl;

    if (impl->store == NULL) {
        impl->store = tiku_trk_store_xattr_open();
    }
    return impl->store;
}

static void
px_close(tiku_trk_backend_t *b)
{
    posix_impl_t *impl = b->impl;
...
static int
px_setperm(tiku_trk_backend_t *b, const char *path, unsigned perm, char *err,
           size_t errmax)
{
    struct stat st;
    mode_t mode;
    ...
    if (perm & TIKU_TRK_P_READ)  { mode |= S_IRUSR; }
    if (perm & TIKU_TRK_P_WRITE) { mode |= S_IWUSR; }
    if (perm & TIKU_TRK_P_EXEC)  { mode |= S_IXUSR; }
...
static const tiku_trk_backend_ops_t POSIX_OPS = {
...
tiku_trk_backend_t *
tiku_trk_backend_posix_open(void)
{
    tiku_trk_backend_t *b = calloc(1, sizeof *b);
