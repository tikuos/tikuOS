 ...
 *
 * tiku_trk_query.h - searching the namespace.
 *
 ...
 */
#ifndef TIKU_TRK_QUERY_H_
#define TIKU_TRK_QUERY_H_

#include "tiku_trk_attr.h"
#include "tiku_trk_model.h"

#include <stdint.h>

#define TIKU_TRK_QUERY_TERMS_MAX   6
#define TIKU_TRK_QUERY_SCOPES_MAX  4
#define TIKU_TRK_QUERY_VALUE_MAX  64
#define TIKU_TRK_QUERY_PRED_MAX  512
...
typedef enum {
    ...
    TIKU_TRK_QF_SIZE,
    TIKU_TRK_QF_CAP,
    TIKU_TRK_QF_UNIT,        /* the descriptor's unit: bytes, Hz, s ...    */
    TIKU_TRK_QF_WRITABLE,
    /* The entry's TYPE, matched as a type rather than as text: picking a
     * supertype admits everything under it (Q-004). */
    TIKU_TRK_QF_TYPE,
    TIKU_TRK_QF_MODIFIED     /* seconds since epoch, with date syntax     */
} tiku_trk_qfield_t;

/** @brief The operator vocabulary, in the original's order. */
typedef enum {
    TIKU_TRK_OP_CONTAINS = 0,
    TIKU_TRK_OP_IS,
    TIKU_TRK_OP_IS_NOT,
    TIKU_TRK_OP_BEGINS_WITH,
    TIKU_TRK_OP_ENDS_WITH,
    TIKU_TRK_OP_GREATER,
    TIKU_TRK_OP_LESS
} tiku_trk_qop_t;
...
typedef struct {
    ...
    tiku_trk_qop_t    op;
    char              value[TIKU_TRK_QUERY_VALUE_MAX];
    tiku_trk_qjoin_t  join;      /* how it joins the PREVIOUS term         */
    int               pattern_ready; /* formula already supplied a glob   */
} tiku_trk_qterm_t;

/** @brief What the panel is asking. */
typedef enum {
    TIKU_TRK_QMODE_NAME = 0,
    TIKU_TRK_QMODE_ATTRIBUTE,
    TIKU_TRK_QMODE_FORMULA
} tiku_trk_qmode_t;

/** @brief What a saved record IS, which decides what opening it does. */
typedef enum {
    TIKU_TRK_QREC_QUERY = 0,     /* opening it runs it                    */
    TIKU_TRK_QREC_TEMPLATE       /* opening it opens the panel, filled in */
} tiku_trk_qrec_t;
...
typedef struct {
    ...
    int              include_trash;
    char             trash_path[TIKU_TRK_PATH_MAX];
...
void tiku_trk_query_init(tiku_trk_query_t *q);
 ...
 */
void tiku_trk_query_set_name(tiku_trk_query_t *q, const char *text);

/** @brief Parse the predicate grammar emitted by query_predicate(). */
int tiku_trk_query_set_formula(tiku_trk_query_t *q, const char *formula);

/** @brief Append a term.  @return its index, or -1. */
int tiku_trk_query_add_term(tiku_trk_query_t *q, tiku_trk_qfield_t field,
                            tiku_trk_qop_t op, const char *value,
                            tiku_trk_qjoin_t join);

/** @brief Restrict results to a subtree.  @return 1 when it was added. */
int tiku_trk_query_add_scope(tiku_trk_query_t *q, const char *path);

void tiku_trk_query_clear_scope(tiku_trk_query_t *q);

/** @brief Record the current identities of scopes that have none yet. */
void tiku_trk_query_bind_scopes(tiku_trk_query_t *q,
 ...
 */
int tiku_trk_query_glob(const char *pattern, const char *name);
 ...
 * also finds "foo-bar".
 */
int tiku_trk_query_pattern(const char *value, tiku_trk_qop_t op, char *out,
                           size_t max);
...
typedef struct tiku_trk_qrun tiku_trk_qrun_t;

/** @brief Begin a search.  Results arrive as the walk proceeds. */
tiku_trk_qrun_t *tiku_trk_query_start(tiku_trk_backend_t *b,
                                      const tiku_trk_query_t *q);

void tiku_trk_query_stop(tiku_trk_qrun_t *r);
 ...
 */
int tiku_trk_query_step(tiku_trk_qrun_t *r, int budget);

int tiku_trk_query_count(const tiku_trk_qrun_t *r);

const tiku_trk_model_t *tiku_trk_query_at(const tiku_trk_qrun_t *r, int i);

/** @brief How many directories have been visited, for a progress line. */
int tiku_trk_query_visited(const tiku_trk_qrun_t *r);

int tiku_trk_query_done(const tiku_trk_qrun_t *r);

/** @brief Whether this live run searches the volume identified by @p dev. */
int tiku_trk_query_targets_device(const tiku_trk_qrun_t *r, uint64_t dev);

/** @brief Type whose results layout this run shares, or empty for any type. */
const char *tiku_trk_query_search_type(const tiku_trk_qrun_t *r);
 ...
 */
typedef enum {
    TIKU_TRK_QGONE_UNMATCHED = 0,   /* still there, no longer qualifies   */
    TIKU_TRK_QGONE_VANISHED         /* the node itself has gone           */
} tiku_trk_qgone_t;
 ...
 */
int tiku_trk_query_refresh(tiku_trk_qrun_t *r, tiku_trk_qdiff_t *out);

/** @brief Whether result @p i arrived in the most recent refresh. */
int tiku_trk_query_is_new(const tiku_trk_qrun_t *r, int i);
...
#define TIKU_TRK_QUERY_STORE_NODE "\x01queries"
#define TIKU_TRK_QUERY_ATTR       "_trk/query"
#define TIKU_TRK_QUERY_INDEX_ATTR "_trk/query_index"

struct tiku_trk_store;
...
int tiku_trk_query_serialise(const tiku_trk_query_t *q, char *buf,
                             size_t max);

int tiku_trk_query_save(const tiku_trk_query_t *q, const char *name,
                        struct tiku_trk_store *store);

/** @brief Read back a saved query.  @return 0, or -1 when absent. */
int tiku_trk_query_load(tiku_trk_query_t *q, const char *name,
                        struct tiku_trk_store *store);

/** @brief How many saved records the index is read into at once. */
#define TIKU_TRK_QUERY_LIST_MAX 32
 ...
 */
int tiku_trk_query_list(struct tiku_trk_store *store, char names[][128],
                        int max);

/** @brief Forget a saved query. */
int tiku_trk_query_forget(const char *name, struct tiku_trk_store *store);
 ...
 * @return how many were written to @p names.
 */
int tiku_trk_query_list_kind(struct tiku_trk_store *store,
                             tiku_trk_qrec_t kind, char names[][128],
                             int max);

/** @brief How old a temporary query may get before the sweep takes it. */
#define TIKU_TRK_QUERY_STALE_SEC  (7 * 24 * 60 * 60)

/** @brief How long after a start the sweep holds off (AW-090). */
#define TIKU_TRK_QUERY_SWEEP_WAIT (30 * 60)
 ...
 */
int tiku_trk_query_sweep(struct tiku_trk_store *store, int64_t now,
                         int64_t up_for, const char *const *open, int nopen);

#endif /* TIKU_TRK_QUERY_H_ */
