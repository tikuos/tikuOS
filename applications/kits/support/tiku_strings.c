...
/* The baked English, generated by TikuTracker/tools/genstrings.py from
 * TikuTracker/strings/en.xml.  The catalogue and its generator live in
 * the tracker's repository rather than this one, so nothing here
 * rebuilds this header: it is checked in, and regenerated by hand from
 * over there whenever the English changes.  It is sorted by key, so a
