 ...
 *
 * tiku_trk_strings.c - the words the interface is made of (see header).
 *
...
#include "tiku_trk_strings_default.h"
...
def_cmp(const void *key, const void *entry)
{
    return strcmp((const char *)key,
                  ((const tiku_trk_str_def_t *)entry)->key);
}
...
{
    const tiku_trk_str_def_t *hit = bsearch(key, tiku_trk_str_defaults,
        (size_t)tiku_trk_str_default_count, sizeof tiku_trk_str_defaults[0],
        def_cmp);
...
int
tiku_trk_strings_builtin_count(void)
{
    return tiku_trk_str_default_count;
}
...
int
tiku_trk_strings_load_file(const char *path)
{
    FILE *f;
    long size;
    char *text;
...
const char *
tiku_trk_strings_init(const char *dir, const char *lang)
{
    char chosen[16] = "";
    char path[1024];
    const char *home;

    tiku_trk_strings_reset();
    if (lang != NULL && lang[0] != '\0') {
        short_lang(lang, chosen, sizeof chosen);
    } else {
        const char *env = getenv("TIKU_DESK_LANG");
    ...
    if (dir != NULL && dir[0] != '\0') {
        ...
        if (tiku_trk_strings_load_file(path) > 0) {
            snprintf(g_lang, sizeof g_lang, "%s", chosen);
            return g_lang;
        }
    }
    {
        const char *env = getenv("TIKU_DESK_STRINGS");
