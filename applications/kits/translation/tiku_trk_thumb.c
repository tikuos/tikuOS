 ...
 *
 * tiku_trk_thumb.c - the thumbnail rules and the two queues.
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include "tiku_trk_thumb.h"
...
void
tiku_trk_thumb_init(tiku_trk_thumb_q_t *q)
{
    if (q != NULL) {
        memset(q, 0, sizeof *q);
...
int
tiku_trk_thumb_wanted(const tiku_trk_thumb_q_t *q, const char *path)
{
    if (q == NULL || !q->enabled) {
        return 0;
    }
    return tiku_trk_image_is_image(path);
}

int
tiku_trk_thumb_fresh(tiku_trk_store_t *store, const char *path, int64_t mtime)
{
    int64_t made = 0;
    ...
    if (tiku_trk_state_read(store, path, TIKU_TRK_ATTR_THUMB_TIME, &made,
...
static int
in_flight(const tiku_trk_thumb_q_t *q, const char *path)
{
    int i;
...
int
tiku_trk_thumb_queue(tiku_trk_thumb_q_t *q, const char *path, int64_t bytes,
                     int size)
{
    tiku_trk_thumb_job_t *slot;
    ...
    if (bytes >= TIKU_TRK_THUMB_BIG) {
        if (q->nbig >= TIKU_TRK_THUMB_QUEUE) {
            return -1;
        }
        slot = &q->big[q->nbig++];
    } else {
        if (q->nsmall >= TIKU_TRK_THUMB_QUEUE) {
            return -1;
        }
    ...
    slot->bytes = bytes;
    slot->size = (size > 0) ? size : TIKU_TRK_THUMB_SIZE;
    return 1;
}

/** @brief Take the front job off @p list. */
static int
pop(tiku_trk_thumb_job_t *list, int *n, tiku_trk_thumb_job_t *out)
{
    int i;
...
int
tiku_trk_thumb_step(tiku_trk_thumb_q_t *q, tiku_trk_store_t *store)
{
    tiku_trk_thumb_job_t job;
    tiku_trk_image_t src, thumb;
    struct stat st;
    int64_t now_mtime = 0;
    ...
    if (tiku_trk_image_thumb(&src, TIKU_TRK_THUMB_SIZE, &thumb) == 0) {
        if (store != NULL) {
                ...
                {   /* The write conjures the cache directory; nothing
                    ...
                    if (home != NULL &&
                        snprintf(dir, sizeof dir, "%s/.cache/tiku-tracker",
                                 home) > 0) {
                        (void)tiku_trk_state_mkparents(dir);
                        (void)mkdir(dir, 0755);
                    }
                }
            ...
            (void)tiku_trk_state_write(store, job.path,
                                       TIKU_TRK_ATTR_THUMB_DIM, dim,
                                       sizeof dim);
            if (stat(job.path, &st) == 0) {
                now_mtime = (int64_t)st.st_mtime + 1;
            }
            (void)tiku_trk_state_write(store, job.path,
                                       TIKU_TRK_ATTR_THUMB_TIME, &now_mtime,
                                       sizeof now_mtime);
        }
        q->made++;
        tiku_trk_image_free(&thumb);
    }
    tiku_trk_image_free(&src);
    q->active[0] = '\0';
    return 1;
}

int
tiku_trk_thumb_load(tiku_trk_store_t *store, const char *path,
                    tiku_trk_image_t *out)
{
    size_t want = (size_t)TIKU_TRK_THUMB_SIZE * TIKU_TRK_THUMB_SIZE *
                  sizeof(uint32_t);
    ...
    out->w = TIKU_TRK_THUMB_SIZE;
    out->h = TIKU_TRK_THUMB_SIZE;
    return 0;
}
