 ...
 *
 * tiku_trk_iconpaint.c - compositing a rendered icon onto a surface.
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include "tiku_trk_iconpaint.h"
#include "tiku_trk_icons.h"

/** @brief Blend one premultiplied source pixel over an opaque destination. */
static tiku_desk_rgb_t
over(uint32_t src, tiku_desk_rgb_t dst)
{
    ...
    if (g > 255u) { g = 255u; }
    if (b > 255u) { b = 255u; }
    return TIKU_DESK_RGB(r, g, b);
}

/** @brief Composite an already-derived bitmap: the cached-look path. */
static int
paint_bmp(tiku_desk_surface_t *s, const uint32_t *bmp, int x, int y,
          int size)
{
    int row, col;
    ...
    for (row = 0; row < size; row++) {
        ...
        for (col = 0; col < size; col++) {
            ...
            tiku_desk_pixel(s, px, py, over(src, tiku_desk_peek(s, px, py)));
        }
    }
    return 1;
}
...
static int
paint(tiku_desk_surface_t *s, const char *name, int x, int y, int size,
      float mix, tiku_desk_rgb_t wash)
{
    const uint32_t *bmp;
    int row, col;
    unsigned m;
    ...
    bmp = tiku_trk_icons_bitmap(name, size);
    if (bmp == NULL) {
        return 0;
    }
    ...
    for (row = 0; row < size; row++) {
        ...
        for (col = 0; col < size; col++) {
            ...
            d = over(src, tiku_desk_peek(s, px, py));
            if (m != 0u) {
                ...
                d = TIKU_DESK_RGB(r / 255u, g / 255u, b / 255u);
            }
            tiku_desk_pixel(s, px, py, d);
        }
    }
    return 1;
}
...
int
tiku_trk_icon_paint(tiku_desk_surface_t *s, const char *name, int x, int y,
                    int size)
{
...
int
tiku_trk_icon_paint_dim(tiku_desk_surface_t *s, const char *name, int x, int y,
                        int size, float mix, tiku_desk_rgb_t wash)
{
    return paint(s, name, x, y, size, mix, wash);
}

int
tiku_trk_icon_paint_selected(tiku_desk_surface_t *s, const char *name, int x,
                             int y, int size)
{
    ...
    {
        const uint32_t *w = tiku_trk_icons_bitmap_washed(name, size,
            0x000000u, 87u);
    ...
    return paint(s, name, x, y, size, 0.341f, TIKU_DESK_RGB(0, 0, 0));
}

void
tiku_trk_thumb_paint(tiku_desk_surface_t *s, const tiku_trk_image_t *th,
                     int x, int y, int size)
{
    ...
    for (py = 0; py < size; py++) {
        ...
        for (px = 0; px < size; px++) {
            ...
            if ((p >> 24) != 0u) {
                tiku_desk_pixel(s, x + px, y + py,
                                TIKU_DESK_RGB((p >> 16) & 0xffu,
                                              (p >> 8) & 0xffu, p & 0xffu));
            }
        }
    }
...
int
tiku_trk_icon_hit(const char *name, int size, int px, int py)
{
    const uint32_t *bm;
    ...
    bm = tiku_trk_icons_bitmap(name, size);
    if (bm == NULL) {
...
void
tiku_trk_paint_tint(tiku_desk_surface_t *s, tiku_desk_rect_t r,
                    tiku_desk_rgb_t c, int a)
{
    int x, y;
    ...
    for (y = r.y; y < r.y + r.h; y++) {
        ...
        for (x = r.x; x < r.x + r.w; x++) {
            ...
            tiku_desk_pixel(s, x, y, TIKU_DESK_RGB(pr, pg, pb));
        }
    }
}

void
tiku_trk_paint_band(tiku_desk_surface_t *s, tiku_desk_rect_t r,
                    int transparent)
{
    ...
    }
    {
        ...
        tiku_desk_rect_t in = { r.x + 1, r.y + 1, r.w - 2, r.h - 2 };
        tiku_desk_rect_t e;

        e = (tiku_desk_rect_t){ r.x, r.y, r.w, 1 };
        tiku_trk_paint_tint(s, e, TIKU_DESK_C_FOCUS, 128);
        e = (tiku_desk_rect_t){ r.x, r.y + r.h - 1, r.w, 1 };
        tiku_trk_paint_tint(s, e, TIKU_DESK_C_FOCUS, 128);
        e = (tiku_desk_rect_t){ r.x, r.y, 1, r.h };
        tiku_trk_paint_tint(s, e, TIKU_DESK_C_FOCUS, 128);
        e = (tiku_desk_rect_t){ r.x + r.w - 1, r.y, 1, r.h };
        tiku_trk_paint_tint(s, e, TIKU_DESK_C_FOCUS, 128);
        if (in.w > 0 && in.h > 0) {
            tiku_trk_paint_tint(s, in, TIKU_DESK_C_SELECT, 90);
        }
    }
}

void
tiku_trk_paint_image(tiku_desk_surface_t *s, const tiku_trk_image_t *im,
                     tiku_desk_rect_t dst, tiku_desk_rect_t clip)
{
    int px, py;
...
void
tiku_trk_paint_text_halo(tiku_desk_surface_t *s, const tiku_desk_font_t *f,
                         int x, int y, const char *text, tiku_desk_rgb_t ink)
{
    ...
    tiku_desk_rgb_t halo = (luminance > 127) ? TIKU_DESK_RGB(0, 0, 0)
                                             : TIKU_DESK_RGB(255, 255, 255);
    int i;
    ...
    for (i = 0; i < 8; i++) {
        tiku_desk_text(s, f, x + dx[i], y + dy[i], text, halo);
    }
    tiku_desk_text(s, f, x, y, text, ink);
}
