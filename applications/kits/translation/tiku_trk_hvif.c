 ...
 *
 * tiku_trk_hvif.c - HVIF parser and rasteriser.
 *
...
static int
hvif_read_styles(hvif_rd_t *r, struct tiku_trk_hvif *ic, char *err,
                 size_t errlen)
{
    int n = hvif_u8(r), i;
...
static int
hvif_read_paths(hvif_rd_t *r, struct tiku_trk_hvif *ic, char *err,
                size_t errlen)
{
    int n = hvif_u8(r), i;
...
static int
hvif_read_shapes(hvif_rd_t *r, struct tiku_trk_hvif *ic, char *err,
                 size_t errlen)
{
    int n = hvif_u8(r), i;
...
tiku_trk_hvif_t *
tiku_trk_hvif_parse(const void *data, size_t len, char *err, size_t errlen)
{
    static const uint8_t magic[4] = { 0x6E, 0x63, 0x69, 0x66 };   /* "ncif" */
    char scratch[TIKU_TRK_HVIF_ERRLEN];
    struct tiku_trk_hvif *ic;
    hvif_rd_t r;
...
void
tiku_trk_hvif_free(tiku_trk_hvif_t *icon)
{
    int i;
...
int
tiku_trk_hvif_style_count(const tiku_trk_hvif_t *icon)
{
    return icon != NULL ? icon->nstyle : 0;
}

int
tiku_trk_hvif_path_count(const tiku_trk_hvif_t *icon)
{
    return icon != NULL ? icon->npath : 0;
}

int
tiku_trk_hvif_shape_count(const tiku_trk_hvif_t *icon)
{
...
int
tiku_trk_hvif_render(const tiku_trk_hvif_t *icon, uint32_t *bmp, int w, int h)
{
    hvif_raster_t r;
    hvif_poly_t poly;
    float g[6], scale;
...
void
tiku_trk_hvif_blit(tiku_desk_surface_t *s, int dx, int dy, const uint32_t *bmp,
                   int w, int h)
{
    int x, y, x0, y0, x1, y1;
    ...
    for (y = y0; y < y1; y++) {
        for (x = x0; x < x1; x++) {
            ...
            tiku_desk_rgb_t d = tiku_desk_peek(s, x, y);
