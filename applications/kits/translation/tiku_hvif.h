 ...
 *
 * tiku_hvif.h - the Haiku Vector Icon Format, parsed and rasterised.
 *
 ...
 * License (Be Incorporated, 1991-2000); see tiku_icons.h.
 *
...
typedef struct tiku_hvif tiku_hvif_t;

/** @brief Enough room for any message tiku_hvif_parse() writes. */
#define TIKU_HVIF_ERRLEN 96
 ...
 */
tiku_hvif_t *tiku_hvif_parse(const void *data, size_t len,
                                     char *err, size_t errlen);

/** @brief Release an icon from tiku_hvif_parse(). */
void tiku_hvif_free(tiku_hvif_t *icon);
 ...
 * already multiplied by alpha -- the layout tiku_rgb_t uses, plus a
 ...
 */
int tiku_hvif_render(const tiku_hvif_t *icon, uint32_t *bmp,
                         int w, int h);
 ...
 * @param bmp Premultiplied 0xAARRGGBB pixels from tiku_hvif_render().
 * @param w   Bitmap width.
 * @param h   Bitmap height.
 */
void tiku_hvif_blit(tiku_surface_t *s, int dx, int dy,
                        const uint32_t *bmp, int w, int h);

/** @brief Number of styles the icon carries.  0 if @p icon is NULL. */
int tiku_hvif_style_count(const tiku_hvif_t *icon);

/** @brief Number of paths the icon carries.  0 if @p icon is NULL. */
int tiku_hvif_path_count(const tiku_hvif_t *icon);

/** @brief Number of shapes the icon carries.  0 if @p icon is NULL. */
int tiku_hvif_shape_count(const tiku_hvif_t *icon);
