 ...
 *
 * tiku_trk_fontdir.c - the faces a person dropped in (see the header).
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include "tiku_trk_fontdir.h"
...
static tiku_trk_fontdir_entry_t *
slot_for(tiku_trk_fontdir_t *out, const char *name)
{
    int i;
    ...
    if (out->count >= TIKU_TRK_FONTDIR_MAX) {
        return NULL;
    }
    {
        tiku_trk_fontdir_entry_t *e = &out->font[out->count++];
...
int
tiku_trk_fontdir_scan(tiku_trk_fontdir_t *out, const char *dir)
{
    struct dirent *entry;
    DIR *d;
    ...
    while ((entry = readdir(d)) != NULL) {
        ...
        if (!tiku_desk_ttf_is_font(path)) {
            continue;
        }
        ttf = tiku_desk_ttf_open(path);
        if (ttf == NULL) {
            continue;
        }
        family = tiku_desk_ttf_family(ttf);
        slot = (family != NULL && family[0] != '\0') ? slot_for(out, family)
                                                     : NULL;
        if (slot != NULL) {
            char *into = tiku_desk_ttf_bold(ttf) ? slot->bold : slot->regular;

            if (into[0] == '\0') {
                snprintf(into, TIKU_TRK_PATH_MAX, "%s", path);
            }
        }
...

    {
        ...
        for (i = 0; i < out->count; i++) {
            if (out->font[i].regular[0] == '\0') {
                snprintf(out->font[i].regular, TIKU_TRK_PATH_MAX, "%s",
                         out->font[i].bold);
            }
        }
        ...
        for (i = 1; i < out->count; i++) {
            tiku_trk_fontdir_entry_t hold = out->font[i];
...
const tiku_trk_fontdir_entry_t *
tiku_trk_fontdir_find(const tiku_trk_fontdir_t *dir, const char *name)
{
    int i;
