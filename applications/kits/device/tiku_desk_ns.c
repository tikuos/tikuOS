 ...
 *
 * tiku_desk_ns.c - manifest parsing and the mirrored namespace.
 *
...
struct tiku_desk_ns {
    tiku_desk_session_t *s;
    tiku_desk_node_t    *nodes;
    int                  count;
    int                  cap;
    int                  truncated;       /* manifest cut by the buffer */
    int                  has_push;        /* device answered `sub`     */
    int                  push_probed;
    char                 pending[NS_PENDING_MAX][TIKU_DESK_PATH_MAX];
    int                  npending;
};
...
ns_notify(const char *path, void *ctx)
{
    ...
    if (ns->npending < NS_PENDING_MAX) {
        snprintf(ns->pending[ns->npending], TIKU_DESK_PATH_MAX, "%s", path);
        ns->npending++;
    }
}

tiku_desk_ns_t *
tiku_desk_ns_new(tiku_desk_session_t *s)
{
    tiku_desk_ns_t *ns;
    ...
    tiku_desk_session_on_notify(s, ns_notify, ns);
    return ns;
}

void
tiku_desk_ns_free(tiku_desk_ns_t *ns)
{
    if (ns == NULL) {
        return;
    }
...
static tiku_desk_node_t *
ns_grow(tiku_desk_ns_t *ns)
{
    if (ns->count == ns->cap) {
        int ncap = ns->cap * 2;
        tiku_desk_node_t *p = realloc(ns->nodes,
...
int
tiku_desk_ns_load(tiku_desk_ns_t *ns)
{
    char *buf;
    const char *p;
    int rc;
    ...
    rc = tiku_desk_session_cmd(ns->s, "cat " NS_MANIFEST_PATH, buf,
                               NS_READ_BUF, 15000);
    ...
    for (p = buf; *p != '\0'; ) {
        ...
        n->is_dir = (kind[0] == 'd');
        n->perm = 0u;
        if (perm[0] == 'r') { n->perm |= TIKU_DESK_P_READ; }
        if (perm[1] == 'w') { n->perm |= TIKU_DESK_P_WRITE; }
...
int
tiku_desk_ns_events(tiku_desk_ns_t *ns, tiku_desk_change_t *out, int max,
                    unsigned *dropped)
{
    char buf[1024];
    char *line, *save;
    ...
    rc = tiku_desk_session_cmd(ns->s, "cat /sys/vfs/events", buf,
                               sizeof buf, 4000);
    if (rc <= 0) {
...
const tiku_desk_node_t *
tiku_desk_ns_find_id(const tiku_desk_ns_t *ns, unsigned id)
{
    int i;
...
int
tiku_desk_ns_count(const tiku_desk_ns_t *ns)
{
    return (ns != NULL) ? ns->count : 0;
}

const tiku_desk_node_t *
tiku_desk_ns_at(const tiku_desk_ns_t *ns, int i)
{
...
static tiku_desk_node_t *
ns_find_mut(tiku_desk_ns_t *ns, const char *path)
{
    int i;
...
const tiku_desk_node_t *
tiku_desk_ns_find(const tiku_desk_ns_t *ns, const char *path)
{
    return ns_find_mut((tiku_desk_ns_t *)ns, path);
}

int
tiku_desk_ns_children(const tiku_desk_ns_t *ns, const char *path,
                      const tiku_desk_node_t **out, int max)
{
    size_t plen;
    int i, n = 0;
    int root;
    ...
    for (i = 0; i < ns->count && n < max; i++) {
        const tiku_desk_node_t *c = &ns->nodes[i];
        const char *rest;
...
int
tiku_desk_ns_ls_kinds(tiku_desk_ns_t *ns, const char *path, char out[][64],
                      char *is_dir, int max)
{
    char buf[8192];
    char cmd[TIKU_DESK_PATH_MAX + 8];
    const char *p;
    int n = 0;
    ...
    if (tiku_desk_session_cmd(ns->s, cmd, buf, sizeof buf, 6000) < 0) {
        return -1;
    }
...
int
tiku_desk_ns_ls(tiku_desk_ns_t *ns, const char *path, char out[][64], int max)
{
    return tiku_desk_ns_ls_kinds(ns, path, out, NULL, max);
}

int
tiku_desk_ns_read(tiku_desk_ns_t *ns, const char *path)
{
    tiku_desk_node_t *n = ns_find_mut(ns, path);
    char cmd[TIKU_DESK_PATH_MAX + 8];
    char buf[TIKU_DESK_VAL_MAX];
    int rc;

    if (n == NULL || (n->perm & TIKU_DESK_P_READ) == 0u) {
        return -1;
    }
    snprintf(cmd, sizeof cmd, "cat %s", path);
    rc = tiku_desk_session_cmd(ns->s, cmd, buf, sizeof buf, 6000);
    if (rc < 0 || strstr(buf, "cannot read") != NULL) {
        n->value_valid = 0;
        return -1;
    }
...
int
tiku_desk_ns_write(tiku_desk_ns_t *ns, const char *path, const char *value,
                   char *err, size_t errmax)
{
    char cmd[TIKU_DESK_PATH_MAX + TIKU_DESK_VAL_MAX + 16];
    char buf[512];
    int rc;
    ...
    rc = tiku_desk_session_cmd(ns->s, cmd, buf, sizeof buf, 20000);
    if (rc < 0) {
        return -1;
    }
    ...
    (void)tiku_desk_ns_read(ns, path);
    return 0;
}

int
tiku_desk_ns_subscribe(tiku_desk_ns_t *ns, const char *path)
{
    tiku_desk_node_t *n = ns_find_mut(ns, path);
    char cmd[TIKU_DESK_PATH_MAX + 16];
    char buf[256];
    ...
    if (tiku_desk_session_cmd(ns->s, cmd, buf, sizeof buf, 4000) < 0) {
        return -1;
    }
    ns->push_probed = 1;
...
int
tiku_desk_ns_unsubscribe(tiku_desk_ns_t *ns, const char *path)
{
    tiku_desk_node_t *n = ns_find_mut(ns, path);
    char cmd[TIKU_DESK_PATH_MAX + 16];
    ...
    return (tiku_desk_session_cmd(ns->s, cmd, NULL, 0u, 4000) < 0) ? -1 : 0;
}

int
tiku_desk_ns_truncated(const tiku_desk_ns_t *ns)
{
...
static tiku_desk_node_t *
ns_add_walked(tiku_desk_ns_t *ns, const char *parent, const char *name,
              int is_dir)
{
    tiku_desk_node_t *n;
    char path[TIKU_DESK_PATH_MAX];
    ...
    n->is_dir = is_dir;
    n->perm = TIKU_DESK_P_READ;           /* refined by the first read */
    n->depth = path_depth(n->path);
    set_name(n);
    return n;
}
 ...
 *
 * `ls` marks directories with a trailing slash, which tiku_desk_ns_ls strips,
 * so kind is re-derived by asking whether the entry lists as a directory.
 */
static int
ns_walk(tiku_desk_ns_t *ns, const char *path, int depth, int max_depth)
{
    char names[64][64];
    char kinds[64];
    int n, i, added = 0;
    ...
    n = tiku_desk_ns_ls_kinds(ns, path, names, kinds, 64);
    if (n < 0) {
        return 0;
    }
    for (i = 0; i < n; i++) {
...
int
tiku_desk_ns_complete(tiku_desk_ns_t *ns, int max_depth)
{
    if (ns == NULL) {
        return -1;
    }
...
int
tiku_desk_ns_has_push(const tiku_desk_ns_t *ns)
{
    return (ns != NULL) ? ns->has_push : 0;
}

int
tiku_desk_ns_pump(tiku_desk_ns_t *ns, int timeout_ms)
{
    int i, refreshed = 0;
    ...
    if (!ns->has_push) {
        ...
        for (i = 0; i < ns->count; i++) {
            if (ns->nodes[i].subscribed &&
                tiku_desk_ns_read(ns, ns->nodes[i].path) == 0) {
                refreshed++;
            }
        }
    }
