 ...
 *
 * tiku_desk_ns.h - the device's namespace, mirrored host-side.
 *
 ...
 */
#ifndef TIKU_DESK_NS_H_
#define TIKU_DESK_NS_H_

#include <stddef.h>
#include "tiku_desk_session.h"

#define TIKU_DESK_PATH_MAX  128
...
typedef struct {
    ...
    unsigned id;
    unsigned perm;                        /* TIKU_DESK_P_*                 */
    int      is_dir;
    int      depth;                       /* '/' components, root = 0      */
    int      subscribed;
    char     value[TIKU_DESK_VAL_MAX];    /* last read; empty until read   */
    int      value_valid;
    unsigned generation;                  /* bumped on every value change  */
} tiku_desk_node_t;

typedef struct tiku_desk_ns tiku_desk_ns_t;

/** @brief Create a mirror over an established session (not owned). */
tiku_desk_ns_t *tiku_desk_ns_new(tiku_desk_session_t *s);

void tiku_desk_ns_free(tiku_desk_ns_t *ns);
 ...
 */
int tiku_desk_ns_load(tiku_desk_ns_t *ns);

/** @brief Node count in the mirror. */
int tiku_desk_ns_count(const tiku_desk_ns_t *ns);

/** @brief Node by index (0..count-1), or NULL. */
const tiku_desk_node_t *tiku_desk_ns_at(const tiku_desk_ns_t *ns, int i);
 ...
 * already showing.
 */
const tiku_desk_node_t *tiku_desk_ns_find_id(const tiku_desk_ns_t *ns,
                                             unsigned id);

const tiku_desk_node_t *tiku_desk_ns_find(const tiku_desk_ns_t *ns,
                                          const char *path);
 ...
 */
int tiku_desk_ns_children(const tiku_desk_ns_t *ns, const char *path,
 ...
 */
int tiku_desk_ns_ls(tiku_desk_ns_t *ns, const char *path,
                    char out[][64], int max);

/**
 * @brief As tiku_desk_ns_ls(), also reporting which entries are directories.
 *
 * @param is_dir  Optional parallel array, 1 per directory entry.
 */
int tiku_desk_ns_ls_kinds(tiku_desk_ns_t *ns, const char *path,
 ...
 */
int tiku_desk_ns_events(tiku_desk_ns_t *ns, tiku_desk_change_t *out, int max,
                        unsigned *dropped);
 ...
 */
int tiku_desk_ns_read(tiku_desk_ns_t *ns, const char *path);
 ...
 */
int tiku_desk_ns_write(tiku_desk_ns_t *ns, const char *path, const char *value,
 ...
 */
int tiku_desk_ns_subscribe(tiku_desk_ns_t *ns, const char *path);

/** @brief Drop a subscription (`sub del`).  0 on success. */
int tiku_desk_ns_unsubscribe(tiku_desk_ns_t *ns, const char *path);

/**
 * @brief Whether the manifest arrived cut by the device's read buffer.
 *
 * True means the mirror covers only the nodes that fit; tiku_desk_ns_complete()
 * fills the rest from `ls`.
 */
int tiku_desk_ns_truncated(const tiku_desk_ns_t *ns);
 ...
 */
int tiku_desk_ns_complete(tiku_desk_ns_t *ns, int max_depth);

/** @brief Whether the device answered `sub` at all (push vs poll UI). */
int tiku_desk_ns_has_push(const tiku_desk_ns_t *ns);
 ...
 */
int tiku_desk_ns_pump(tiku_desk_ns_t *ns, int timeout_ms);

#endif /* TIKU_DESK_NS_H_ */
