 ...
 *
 * tiku_desk_session.c - prompt discipline, echo removal, notify demux.
 *
...
struct tiku_desk_session {
    tiku_desk_tx_t     *tx;
    unsigned char      *buf;
    size_t              len;
    size_t              cap;
    tiku_desk_notify_fn notify;
    void               *notify_ctx;
};
...
static int
buf_reserve(tiku_desk_session_t *s, size_t extra)
{
 ...
 */
static int
drain_notifies(tiku_desk_session_t *s)
{
    size_t r = 0, w = 0;
    int fired = 0;
    int at_line_start = 1;
...
static int
pump(tiku_desk_session_t *s, int timeout_ms, int *notified)
{
    unsigned char chunk[4096];
    int n = tiku_desk_tx_read(s->tx, chunk, sizeof chunk, timeout_ms);
    int f;
...
tiku_desk_session_t *
tiku_desk_session_new(tiku_desk_tx_t *tx)
{
...
void
tiku_desk_session_free(tiku_desk_session_t *s)
{
    if (s == NULL) {
        return;
    }
    tiku_desk_tx_close(s->tx);
    free(s->buf);
    free(s);
}

void
tiku_desk_session_on_notify(tiku_desk_session_t *s, tiku_desk_notify_fn fn,
                            void *ctx)
{
    if (s != NULL) {
        s->notify = fn;
...
int
tiku_desk_session_sync(tiku_desk_session_t *s, int timeout_ms)
{
    long deadline;
...
int
tiku_desk_session_cmd(tiku_desk_session_t *s, const char *line, char *out,
                      size_t max, int timeout_ms)
{
    ...
    if (tiku_desk_tx_write(s->tx, line, strlen(line)) < 0 ||
        tiku_desk_tx_write(s->tx, "\n", 1u) < 0) {
        return -1;
    }
    echo = strlen(line);
    deadline = now_ms() + timeout_ms;
...
int
tiku_desk_session_poll(tiku_desk_session_t *s, int timeout_ms)
{
    int fired = 0;
...
int
tiku_desk_session_reconnect(tiku_desk_session_t *s)
{
    if (s == NULL || tiku_desk_tx_reopen(s->tx) != 0) {
        return -1;
    }
    s->len = 0;
    return tiku_desk_session_sync(s, 4000);
}

const char *
tiku_desk_session_name(const tiku_desk_session_t *s)
{
    return (s != NULL) ? tiku_desk_tx_name(s->tx) : "(none)";
}
