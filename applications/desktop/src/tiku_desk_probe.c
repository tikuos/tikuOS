 ...
 *
 * tiku_desk_probe.c - headless exercise of the session and namespace layers.
 *
...
static void
print_tree(const tiku_desk_ns_t *ns, const char *path, int indent, int budget)
{
    const tiku_desk_node_t *kids[64];
    int n = tiku_desk_ns_children(ns, path, kids, 64);
    int i;

    for (i = 0; i < n && budget > 0; i++) {
        const tiku_desk_node_t *k = kids[i];
        printf("%*s%s%s%s%s\n", indent, "", k->name,
               k->is_dir ? "/" : "",
               (k->perm & TIKU_DESK_P_WRITE) ? "  [w]" : "",
...
main(int argc, char **argv)
{
    ...
    if (tcp != NULL) {
        ...
        memcpy(host, tcp, hl);
        host[hl] = '\0';
        tx = tiku_desk_tx_open_tcp(host, tport);
    } else {
        if (port == NULL) {
            int found = tiku_desk_tx_scan_serial(scan, 8);
            if (found <= 0) {
    ...
    s = tiku_desk_session_new(tx);
    if (s == NULL) {
        fprintf(stderr, "probe: session\n");
        return 1;
    }
    if (tiku_desk_session_sync(s, 4000) != 0) {
        fprintf(stderr, "probe: no prompt from %s -- wrong baud, or the "
                        "board is busy\n", tiku_desk_session_name(s));
        tiku_desk_session_free(s);
        return 1;
    }
    printf("connected : %s\n", tiku_desk_session_name(s));

    if (tiku_desk_session_cmd(s, "info", buf, sizeof buf, 4000) > 0) {
        char *nl = strchr(buf, '\n');
        if (nl != NULL) { *nl = '\0'; }
        printf("device    : %s\n", buf);
    }

    ns = tiku_desk_ns_new(s);
    count = tiku_desk_ns_load(ns);
    if (count < 0) {
        fprintf(stderr, "probe: no %s on this build\n", "/sys/vfs/manifest");
        tiku_desk_ns_free(ns);
        tiku_desk_session_free(s);
        return 1;
    }
    printf("manifest  : %d nodes%s\n", count,
           tiku_desk_ns_truncated(ns) ? " (CUT by the device read buffer)"
                                      : "");
    if (tiku_desk_ns_truncated(ns)) {
        int added = tiku_desk_ns_complete(ns, 4);
        printf("completed : +%d nodes by ls-walk (no descriptors on those)\n",
               added);
        count = tiku_desk_ns_count(ns);
    }
    {
        int dirs = 0, writable = 0, capped = 0;
        for (i = 0; i < count; i++) {
            const tiku_desk_node_t *n = tiku_desk_ns_at(ns, i);
            if (n->is_dir) { dirs++; }
            if (n->perm & TIKU_DESK_P_WRITE) { writable++; }
    ...
    if (tiku_desk_ns_read(ns, "/sys/uptime") == 0) {
        const tiku_desk_node_t *n = tiku_desk_ns_find(ns, "/sys/uptime");
        printf("uptime    : %s\n", n->value);
    }

    {
        char names[32][64];
        int n = tiku_desk_ns_ls(ns, "/data", names, 32);
        if (n >= 0) {
    ...
    if (wr != NULL) {
        char path[TIKU_DESK_PATH_MAX], err[256];
        ...
        if (eq == NULL) {
            ...
            memcpy(path, wr, pl);
            path[pl] = '\0';
            if (tiku_desk_ns_write(ns, path, eq + 1, err, sizeof err) == 0) {
                const tiku_desk_node_t *n = tiku_desk_ns_find(ns, path);
    ...
    if (sub != NULL) {
        ...
        int rc = tiku_desk_ns_subscribe(ns, sub);
        unsigned gen0;
        ...
        if (rc < 0) {
            ...
            (void)tiku_desk_ns_read(ns, sub);
            n = tiku_desk_ns_find(ns, sub);
            gen0 = (n != NULL) ? n->generation : 0u;
            printf("watching  : %d s -- change the value elsewhere\n", secs);
            for (i = 0; i < secs * 4; i++) {
                if (tiku_desk_ns_pump(ns, 250) < 0) {
                    printf("            link lost; reconnecting\n");
                    if (tiku_desk_session_reconnect(s) != 0) { break; }
                }
                n = tiku_desk_ns_find(ns, sub);
            ...
            (void)tiku_desk_ns_unsubscribe(ns, sub);
        }
    }

    printf("push      : %s\n",
           tiku_desk_ns_has_push(ns) ? "yes (`sub` present)"
                                     : "no (poll fallback)");
    tiku_desk_ns_free(ns);
    tiku_desk_session_free(s);
    return 0;
