 ...
 *
 * tiku_desk_glyphpath.c - one outline, however it was described.
 *
...
float
tiku_desk_hint_x(const tiku_desk_hint_t *hint, float x)
{
    return x * hint->scale;
}

float
tiku_desk_hint_y(const tiku_desk_hint_t *hint, float y)
{
...
tiku_desk_path_t *
tiku_desk_path_new(void)
{
    tiku_desk_path_t *path = calloc(1u, sizeof *path);
    ...
    tiku_desk_path_reset(path);
    return path;
}

void
tiku_desk_path_free(tiku_desk_path_t *path)
{
    if (path != NULL) {
        free(path->edge);
        free(path);
...
void
tiku_desk_path_reset(tiku_desk_path_t *path)
{
    if (path == NULL) {
        return;
    }
...
int
tiku_desk_path_failed(const tiku_desk_path_t *path)
{
...
static void
edge(tiku_desk_path_t *path, float x0, float y0, float x1, float y1)
{
    if (path->failed) {
        return;
    }
...
void
tiku_desk_path_move(tiku_desk_path_t *path, float x, float y)
{
    if (path == NULL) {
        return;
    }
...
void
tiku_desk_path_line(tiku_desk_path_t *path, float x, float y)
{
    if (path == NULL) {
        return;
    }
...
void
tiku_desk_path_quad(tiku_desk_path_t *path, float cx, float cy, float x,
                    float y)
{
    float x0, y0;
    int steps, i;
...
void
tiku_desk_path_cubic(tiku_desk_path_t *path, float ax, float ay, float bx,
...
void
tiku_desk_path_close(tiku_desk_path_t *path)
{
...
void
tiku_desk_path_bounds(const tiku_desk_path_t *path, int *x0, int *y0,
                      int *w, int *h)
{
    int ax, ay, bx, by;
