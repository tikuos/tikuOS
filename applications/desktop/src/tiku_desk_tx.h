 ...
 *
 * tiku_desk_tx.h - transport seam: bytes to and from one device.
 *
 ...
 */
#ifndef TIKU_DESK_TX_H_
#define TIKU_DESK_TX_H_

#include <stddef.h>

/** @brief Transport handle.  Opaque; created by one of the openers below. */
typedef struct tiku_desk_tx tiku_desk_tx_t;
 ...
 */
tiku_desk_tx_t *tiku_desk_tx_open_serial(const char *path, int baud);
 ...
 * @return Handle, or NULL (errno set).
 */
tiku_desk_tx_t *tiku_desk_tx_open_tcp(const char *host, int port);
 ...
 */
int tiku_desk_tx_read(tiku_desk_tx_t *tx, void *buf, size_t max,
                      int timeout_ms);

/** @brief Write @p len bytes; -1 on a broken link. */
int tiku_desk_tx_write(tiku_desk_tx_t *tx, const void *buf, size_t len);

/** @brief Reopen the same endpoint after a replug.  0 on success. */
int tiku_desk_tx_reopen(tiku_desk_tx_t *tx);

/** @brief Human-readable endpoint name, for window titles and logs. */
const char *tiku_desk_tx_name(const tiku_desk_tx_t *tx);

void tiku_desk_tx_close(tiku_desk_tx_t *tx);
 ...
 */
int tiku_desk_tx_scan_serial(char out[][256], int max);

#endif /* TIKU_DESK_TX_H_ */
