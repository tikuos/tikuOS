 ...
 *   3. Free TCM -- the allocator span after statics, below the stack guard.
 *   4. Shared SRAM -- 3 MB at 0x20080000, powered in tiku_crt_early.c;
 *      hosts the large SRAM tier.
 *   5. MRAM -- internal flash for code/rodata above the SBL, reported as
 *      FLASH.  The MRAM mirror of .uninit lives inside this slice but is a
 *      reserved range managed by tiku_mem_arch.c.
 *   6. Peripheral aperture -- APB/AHB at 0x40000000, 256 MB.
 *
...
tiku_region_arch_get_table(tiku_mem_arch_size_t *count) {
    if (s_region_count == 0) {
        ...
        /* Free TCM is separate from the durable overlay and stack guard. */
